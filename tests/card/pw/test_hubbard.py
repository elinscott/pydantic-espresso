"""Test `pydantic_espresso.card.pw.hubbard` classes."""

from pydantic_espresso.card.pw.hubbard import HubbardB, HubbardCard, HubbardJ, HubbardU, HubbardV


def test_hubbard_card() -> None:
    """Test the HubbardCard class."""
    card = HubbardCard(
        kind="atomic",
        parameters=[
            HubbardU(site={"label": "Fe", "manifold": "3d"}, value=5.0),
            HubbardV(
                sites=[{"label": "Fe", "manifold": "3d"}, {"label": "O", "manifold": "2p"}],
                atom_indices=(1, 2),
                value=1.0,
            ),
        ],
