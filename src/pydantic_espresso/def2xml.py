...
def defs_to_xmls() -> None:
    ...
    for xsl_file in def_directory.rglob("*.xsl"):
        xsl_file_in_xml_directory = (xml_directory / xsl_file.parent.relative_to(def_directory)
            / xsl_file.name)
...
def def_to_xml(def_file: Path) -> None:
    ...
    with chdir(xml_subdirectory):
        ...
        try:
            subprocess.run([str(helpdoc), str(copied_def_file)], check=True,
                           stdout=subprocess.DEVNULL)
        except subprocess.CalledProcessError:
            warnings.warn(f"Error running helpdoc for {def_file.relative_to(def_directory)}",
                          stacklevel=2)
