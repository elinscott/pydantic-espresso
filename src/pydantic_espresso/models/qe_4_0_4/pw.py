"""Pydantic model for the input of `pw.x` version `qe-4.0.4`.
...
class ControlNamelist(Namelist):
    ...
    calculation: str = Field(
        "scf",
        description="a string describing the task to be performed: 'scf', 'nscf', 'bands', 'phonon', 'relax', 'md', 'vc-relax', 'vc-md', 'neb', 'smd', 'metadyn' (vc = variable-cell).",
    )
    title: str | None = Field(None, description="reprinted on output.")
    verbosity: str | None = Field(None, description="high' | 'default' | 'low' | 'minimal")
    restart_mode: str = Field(
        "from_scratch",
        description="from_scratch'  : from scratch NEB and SMD only: the starting path is obtained with a linear interpolation between the images specified in the ATOMIC_POSITIONS card. Note that in the linear interpolation periodic boundary conditions ARE NON USED.  'restart'       : from previous interrupted run",
    )
    wf_collect: bool = Field(
        False,
        description="This flag controls the way wavefunctions are stored to disk :  .TRUE.  collect wavefunctions from all processors and store them into the output data directory outdir/prefix.save  .FALSE. do not collect wavefunctions, leave them in temporary local files (one per processor). The resulting format will be readable only by jobs running on the same number of processors and pools. Useful if you do not need the wavefunction or if you want to reduce the I/O or the disk occupancy.",
    )
    nstep: int | None = Field(None, description="number of ionic + electronic steps")
    iprint: int | None = Field(
        None, description="band energies are written every iprint iterations"
    )
    tstress: bool | None = Field(
        None, description="calculate stress. Set to .TRUE. if calculation='vc-md"
    )
    tprnfor: bool | None = Field(
        None, description="print forces. Set to .TRUE. if calculation='relax','md','vc-md"
    )
    dt: float = Field(
        20.0e0,
        description="time step for molecular dynamics, in Rydberg atomic units (1 a.u.=4.8378 * 10^-17 s : beware, CP and FPMD codes use Hartree atomic units, half that much!!!)",
    )
    outdir: Path = Field(
    ...
    wfcdir: Path = Field(
        default_factory=get_tmp_dir,
        description="this directory specifies where to store files generated by each processor (*.wfc{N}, *.igk{N}, etc.). The idea here is to be able to separately store the largest files, while the files necessary for restarting still go into 'outdir' (for now only works for stand alone PW )",
    )
    prefix: str = Field(
        "pwscf", description="prepended to input/output filenames: prefix.wfc, prefix.rho, etc."
    )
    lkpoint_dir: bool = Field(
        True,
        description="If .false. it does not open a subdirectory for each k_point in the prefix.save directory.",
    )
    max_seconds: float | None = Field(None, description="jobs stops after max_seconds CPU time")
    ...
    tefield: bool = Field(
        False,
        description="If .TRUE. a sawlike potential simulating an electric field is added to the bare ionic potential. See variables edir, eamp, emaxpos, eopreg for the form and size of the added potential.",
    )
    dipfield: bool = Field(
        False,
        description="If .TRUE. and tefield=.TRUE. a dipole correction is also added to the bare ionic potential - implements the recipe of L. Bengtsson, PRB 59, 12301 (1999) (https://journals.aps.org/prb/abstract/10.1103/PhysRevB.59.12301). See variables edir, emaxpos, eopreg for the form of the correction, that must be used only in a slab geometry, for surface calculations, with the discontinuity in the empty space.",
    )
    lelfield: bool = Field(
        False,
    ...
    lberry: bool = Field(
        False,
        description="If .TRUE. perform a Berry phase calculation See the header of PW/bp_c_phase.f90 for documentation",
    )
    gdir: int | None = Field(
        None,
...
class SystemNamelist(Namelist):
    ...
    nelec: float | None = Field(
        None,
        description="number of electron in the unit cell (may be noninteger if you wish)  A compensating jellium background is inserted to remove divergences if the cell is not neutral",
    )
    tot_charge: int = Field(
        0,
        description="total system charge. Used only if nelec is unspecified, otherwise it is ignored.",
    )
    ecutwfc: float | None = Field(None, description="kinetic energy cutoff (Ry) for wavefunctions")
    ecutrho: float | None = Field(
        None,
        description="kinetic energy cutoff (Ry) for charge density and potential If there are ultrasoft PP, a larger value than the default is often desirable (ecutrho = 8 to 12 times ecutwfc, typically). If all PP are norm-conserving, you should stick to the default; you may reduce it to spare time, but not by a large amount.",
    )
    nosym: bool = Field(
        False,
        description="if (.TRUE.) symmetry is not used. Note that a k-point grid provided in input is used 'as is'; an automatically generated k-point grid will contain only points in the irreducible BZ of the lattice.  Use with care in low-symmetry large cells if you cannot afford a k-point grid with the correct symmetry.",
    )
    ...
    smearing: str = Field(
        "gaussian",
        description="gaussian', 'gauss': ordinary Gaussian spreading (Default)  'methfessel-paxton', 'm-p', 'mp': Methfessel-Paxton first-order spreading (see PRB 40, 3616 (1989) (https://journals.aps.org/prb/abstract/10.1103/PhysRevB.40.3616)).  'marzari-vanderbilt', 'cold', 'm-v', 'mv': Marzari-Vanderbilt cold smearing (see PRL 82, 3296 (1999) (https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.82.3296))  'fermi-dirac', 'f-d', 'fd': smearing with Fermi-Dirac function",
    )
    nspin: int = Field(
        1,
    ...
    xc_type: str | None = Field(None, description="Exchange-correlation functional")
    lda_plus_u: bool = Field(False, description="")
    starting_ns_eigenvalue: list[float] | None = Field(
        None,
    ...
    emaxpos: float = Field(
        0.5e0,
        description="Position of the maximum of the sawlike potential along crystal axis 'edir', within the  unit cell (see below), 0 < emaxpos < 1 Used only if tefield is .TRUE.",
    )
    eopreg: float = Field(
        0.1e0,
        description="Zone in the unit cell where the sawlike potential decreases. ( see below, 0 < eopreg < 1 ). Used only if tefield is .TRUE.",
    )
    eamp: float | None = Field(
        None,
        description="Amplitude of the electric field (in a.u. = 51.44 10^10 V/m ) The sawlike potential increases with slope 'eamp' in the region from (emaxpos+eopreg-1) to (emaxpos), then decreases to 0 until (emaxpos+eopreg), in units of the crystal vector 'edir'. Important: the change of slope of this potential must be located in the empty region, or else unphysical forces will result. Used only if tefield is .TRUE.",
    )
    constrained_magnetization: str = Field(
        "none",
        description="Used to perform constrained calculations in magnetic systems. Currently available choices:  'none': no constraint  'total': total magnetization is constrained If nspin=4 (noncolin=.True.) constraint is imposed by adding a penalty functional to the total energy:  LAMBDA * SUM_{i} ( magnetization(i) - fixed_magnetization(i) )**2  where the sum over i runs over the three components of the magnetization. Lambda is a real number (see below). If nspin=2 constraint is imposed by defining two Fermi energies for spin up and down. Only fixed_magnetization(3) can be defined in this case.  'atomic': atomic magnetization are constrained to the defined starting magnetization adding a penalty:  LAMBDA * SUM_{i,itype} ( magnetic_moment(i,itype) - mcons(i,itype) )**2  where i runs over the cartesian components (or just z in the collinear case) and itype over the types (1-ntype). mcons(:,:) array is defined from starting_magnetization, (and angle1, angle2 in the non-collinear case). lambda is a real number  'total direction': the angle theta of the total magnetization with the z axis (theta = fixed_magnetization(3)) is constrained:  LAMBDA * ( magnetization(1) - magnetization(3)*tan(theta) )**2  'atomic direction': not all the components of the atomic magnetic moment are constrained but only the cosine of angle1, and the penalty functional is:  LAMBDA * SUM_{itype} ( mag_mom(3,itype)/mag_mom_tot - cos(angle1(ityp)) )**2",
    )
    Lambda: float | None = Field(
        None,
        description="parameter used for constrained_magnetization calculations NB: LAMBDA is reduced in the first iterations and is increased slowly up to the input value.",
    )
    report: int | None = Field(
        None,
        description="It's the number of iterations after which the program write all the atomic magnetic moments.",
    )
    lspinorb: bool | None = Field(
        None,
        description="if .TRUE. the noncollinear code can use a pseudopotential with spin-orbit.",
    )
    assume_isolated: bool = Field(
        False,
        description="if .TRUE. the system is assumed to be isolated (a molecule or cluster in a supercell) and the Makov-Payne correction to the total energy is computed. An estimate of the vacuum level is also calculated so that eigenvalues can be properly aligned.",
    )
    do_ee: bool = Field(
        False,
        description="if .TRUE. the system is embedded the electrostatic environment described in the EE namelist.",
    )
    ...
    B_field: tuple[float, float, float] = Field(
        (0.0e0, 0.0e0, 0.0e0),
        description="A fixed magnetic field defined by the vector B_field is added to the exchange and correlation magnetic field. The three components of the magnetic field are given in Ry. Only B_field(3) can be used if nspin=2.  In all calculations with a finite magnetic field, we print the total energy WITHOUT the B dot M term. In the calculations with the penalty functional we write only the total energy, NOT the penalty functional.",
    )
...
class ElectronsNamelist(Namelist):
    ...
    mixing_mode: str = Field(
        "plain",
        description="plain' :    charge density Broyden mixing  'TF' :       as above, with simple Thomas-Fermi screening (for highly homogeneous systems)  'local-TF':  as above, with local-density-dependent TF screening (for highly inhomogeneous systems)",
    )
    mixing_beta: float = Field(0.7e0, description="mixing factor for self-consistency")
    mixing_ndim: int = Field(8, description="number of iterations used in mixing scheme")
    mixing_fixed_ns: int = Field(
        0,
        description="For LDA+U : number of iterations with fixed ns ( ns is the atomic density appearing in the Hubbard term ).",
    )
    diagonalization: str = Field(
        "david",
        description="david':  Davidson iterative diagonalization with overlap matrix (default). Fast, may in some rare cases fail.  'cg' :    conjugate-gradient-like band-by-band diagonalization Typically slower than 'david' but it uses less memory and is more robust (it fails very seldom)  'david-serial': do not use parallel subspace diagonalization in Davidson algorithm (for testing purposes). The subspace diagonalization in Davidson is performed by a fully distributed-memory parallel algorithm on 4 or more processors, by default. The allocated memory scales down with the number of procs. Procs involved in diagonalization can be changed with input parameter 'ortho_para'. On multicore CPUs often it is convenient to let only one core per CPU to work on linear algebra.",
    )
    ortho_para: int = Field(
        0,
        description="meaningful for diagonalization='david' and parallel executables. The number of processors to be used for the parallel subspace diagonalization algorithm. With the default value (0) the code tries to use as many processors as available. Note that the algorithm uses a square number of processors (4, 9, 16, 25,...), so the actual number of processors used will be the largest square number less or equal to ortho_para (if set) or to the total number of processors (if ortho_para is not set).",
    )
    ...
    diago_david_ndim: int = Field(
        4,
        description="For Davidson diagonalization: dimension of workspace (number of wavefunction packets, at least 2 needed). A larger value may yield a faster algorithm but uses more memory",
    )
    ...
    efield: float = Field(
        0.0e0,
        description="For finite electric field calculations (lelfield == .TRUE.), it defines the intensity of the field in a.u.",
    )
    startingpot: str | None = Field(
        None,
        description="atomic': starting potential from atomic charge superposition ( default for scf, *relax, *md, neb, smd )  'file'  : start from existing 'charge-density.xml' file ( default, only possibility for nscf, bands, phonon )",
    )
    startingwfc: str = Field(
        "atomic",
        description="atomic': start from superposition of atomic orbitals If not enough atomic orbitals are available, fill with random numbers the remaining wfcs The scf typically starts better with this option, but in some high-symmetry cases one can 'loose' valence states, ending up in the wrong ground state.  'atomic+random': as above, plus a superimposed 'randomization' of atomic orbitals. Prevents the 'loss' of states mentioned above.  'random': start from random wfcs. Slower start of scf but safe. It may also reduce memory usage in conjunction with diagonalization='cg'  'file':   start from a wavefunction file",
    )
    tqr: bool = Field(
        False,
        description="If .true., use the (VERY EXPERIMENTAL) real-space algorithm for augmentation charges in ultrasoft pseudopotentials. Must faster execution of ultrasoft-related calculations, but numerically less accurate than the default algorithm. Use with care and after testing!",
    )
...
class IonsNamelist(Namelist):
    ...
    ion_dynamics: str | None = Field(
        None,
        description="Specify the type of ionic dynamics.  For constrained dynamics or constrained optimisations add the CONSTRAINTS card (when the card is present the SHAKE algorithm is automatically used).  For different type of calculation different possibilities are allowed and different default values apply:  CASE ( calculation = 'relax' ) 'bfgs' :   (default)   use BFGS quasi-newton algorithm, based on the trust radius procedure, for structural relaxation 'damp' :               use damped (quick-min Verlet) dynamics for structural relaxation  CASE ( calculation = 'md' ) 'verlet' : (default)   use Verlet algorithm to integrate Newton's equation 'langevin'             ion dynamics is over-damped Langevin  CASE ( calculation = 'vc-relax' ) 'damp' :   (default)   use damped (Beeman) dynamics for structural relaxation 'bfgs' :               use BFGS quasi-newton algorithm for structural relaxation (experimental) cell_dynamics must be 'bfgs' too CASE ( calculation = 'vc-md' ) 'beeman' : (default)   use Beeman algorithm to integrate Newton's equation",
    )
    phase_space: str = Field(
        "full",
        description="full' :           the full phase-space is used for the ionic dynamics.  'coarse-grained' : a coarse-grained phase-space, defined by a set of constraints, is used for the ionic dynamics (used for calculation of free-energy barriers)",
    )
    pot_extrapolation: str = Field(
        "atomic",
        description="Used to extrapolate the potential from preceding ionic steps.  'none'        :  no extrapolation  'atomic'      :  extrapolate the potential as if it was a sum of atomic-like orbitals  'first_order' :  extrapolate the potential with first-order formula  'second_order':  as above, with second order formula",
    )
    wfc_extrapolation: str = Field(
        "none",
        description="Used to extrapolate the wavefunctions from preceding ionic steps.  'none'        :  no extrapolation  'first_order' :  extrapolate the wave-functions with first-order formula - NOT IMPLEMENTED WITH USPP  'second_order':  as above, with second order formula NOT IMPLEMENTED WITH USPP",
    )
...
class PhononNamelist(Namelist):
    """Pydantic model for the `PHONON` namelist."""
...
class EeNamelist(Namelist):
    ...
    which_compensation: str = Field(
        "none",
        description="dcc' : density counter charge correction. The electrostatic problem is solved in open boundary conditions. At variance with the Makov-Payne approach that only estimates an energy correction here the scf potential is corrected as well. Theory described in: I.Dabo, B.Kozinsky, N.E.Singh-Miller and N.Marzari, 'Electrostatic periodic boundary conditions and real-space corrections', Phys.Rev.B 77, 115139 (2008)",
    )
    ecutcoarse: float = Field(
        100,
        description="kinetic energy cutoff defining the grid used for the open boundary correction.",
    )
    mixing_charge_compensation: float = Field(
        1.0, description="scf mixing parameter for the correcting potential."
    )
    n_charge_compensation: int = Field(
        5,
        description="the correcting potential is updated (mixed) every n_charge_compensation iteration only.",
    )
    comp_thr: float = Field(
        1.0e-4,
        description="inclusion of dcc correction begins when scf convergence is better than comp_thr.",
    )
    nlev: int = Field(4, description="number of depth levels used by the multigrid solver.")
...
class PWEspressoInput(EspressoInput):
    ...
    phonon: PhononNamelist = Field(default_factory=lambda: PhononNamelist())
