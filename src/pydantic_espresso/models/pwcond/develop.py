...
class InputcondNamelist(Namelist):
    ...
    save_file: str | None = Field(
        ...
        description=dedent(
            """\
            file where the data necessary for PWCOND are written so that no prefix files of PW are
            longer needed"""
        ),
    )
    ...
    ikind: Literal[0, 1, 2] = Field(
        0,
        description=dedent(
            """\
            The kind of conductance calculation.
            - '0': Just complex band structure (CBS) calculation.
            - '1': Conductance calculation with identical left and right leads.
            - '2': Conductance calculation with different left and right leads."""
        ),
    )
    ...
    nenergy: int | None = Field(
        ...
        description=dedent(
            """\
            number of energies  WARNING: the energy in input file is given in eV taken from Ef, and
            denergy should be negative"""
        ),
    )
    start_e: int = Field(
        1,
    ...
    last_k: int | None = Field(
        ...
        description=dedent(
            """\
            index of the last k-point to be computed. If last_k is bigger than the actual number of
            points in the list, then it will be set to that number."""
        ),
    )
...
class PWCONDInput(EspressoInput):
