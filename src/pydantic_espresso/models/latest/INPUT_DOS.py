"""Pydantic model for the input of `dos.x` version `latest`.
...
class DOSEspressoInput(EspressoInputTemplate):
...
value of the ", description="directory containing the input data, i.e. the same as in pw.x")
    bz_sum: Literal["'smearing'", "'tetrahedra'", "'tetrahedra_lin'", "'tetrahedra_opt'"] = Field("
'smearing' if degauss is given in input;
                        options read from the xml data file otherwise.
         ", description="Keyword selecting  the method for BZ summation. Available options are:")
    ngauss: int = Field( 0
    ...
    degauss: float = Field(..., description="gaussian broadening, Ry (not eV!) (see below)")
    DeltaE: float = Field(..., description="energy grid step (eV)")
    fildos: str = Field(" '", description="output file containing DOS(E)")
