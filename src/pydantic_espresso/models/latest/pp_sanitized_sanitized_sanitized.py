"""Pydantic model for the input of `pp.x` version `latest`.
...
class InputppNamelist(BaseModel):
   """Pydantic model for the `Inputpp` namelist."""

    title: str = Field(" ", description="reprinted on output, CUB files and other places")
...
class PlotNamelist(BaseModel):
   """Pydantic model for the `Plot` namelist."""
    ...
    fileout: str = Field("standard output", description="name of the file to which the plot is written")
    interpolation: Literal["fourier", "bspline"] = Field("fourier", description="Type of interpolation:")


class PPEspressoInput(BaseModel):
    """Pydantic model for the input of `pp.x.`"""

    Inputpp: InputppNamelist
    Plot: PlotNamelist
