"""Pydantic model for the input of `dynmat.x` version `latest`.
...
class InputNamelist(BaseModel):
   """Pydantic model for the `Input` namelist."""
    ...
    axis: int = Field(3, description="indicates the rotation axis for a 1D system (1=Ox, 2=Oy, 3=Oz)")
    ...
    lplasma: bool = Field(False, description="if .true. then calculate Gamma-point mode effective plasma frequencies, automatically triggers lperm = .true.")
    filout: str = Field("dynmat.out", description="output file containing phonon frequencies and normalized phonon displacements (i.e. eigenvectors divided by the square root of the mass and then normalized; they are not orthogonal)")
    fileig: str = Field(" ", description="output file containing phonon frequencies and eigenvectors of the dynamical matrix (they are orthogonal)")
    ...
    loto_2d: bool = Field('.false.', description="set to .true. to activate two-dimensional treatment of LO-TO splitting.")
    el_ph_nsig: int | None = Field(None, description="The number of double-delta smearing values used in an electron-phonon coupling calculation.")
    el_ph_sigma: float | None = Field(None, description="The spacing of double-delta smearing values used in an electron-phonon coupling calculation.")


class DYNMATEspressoInput(BaseModel):
