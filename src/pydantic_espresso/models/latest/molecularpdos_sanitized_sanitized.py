"""Pydantic model for the input of `molecularpdos.x` version `latest`.
...
class InputmopdosNamelist(BaseModel):
   """Pydantic model for the `Inputmopdos` namelist."""
    ...
    i_atmwfc_end_part: int | None = Field(None, description="last atomic wavefunction of the molecular part considered for the projection")
...
class MOLECULARPDOSEspressoInput(BaseModel):
    """Pydantic model for the input of `molecularpdos.x.`"""

    Inputmopdos: InputmopdosNamelist
