"""Pydantic model for the input of `d3hess.x` version `latest`.
...
class InputNamelist(BaseModel):
   """Pydantic model for the `Input` namelist."""
    ...
    filhess: str = Field("prefix.hess", description="output file where the D3 hessian matrix is written (should match dftd3_hess keyword in phonon calculation)")
    step: float | None = Field(None, description="step for numerical differentiation in a.u.")


class D3HESSEspressoInput(BaseModel):
