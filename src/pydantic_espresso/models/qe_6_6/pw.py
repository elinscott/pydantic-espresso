"""Pydantic model for the input of `pw.x` version `qe-6.6`.
...
class ControlNamelist(Namelist):
    ...
    wf_collect: bool = Field(
        True,
        description="This flag controls the way wavefunctions are stored to disk :  .TRUE.  collect wavefunctions from all processors, store them into the output data directory 'outdir'/'prefix'.save The resulting format is portable to a different number of processor, or different kind of parallelization  .FALSE. OBSOLETE - NO LONGER IMPLEMENTED do not collect wavefunctions, leave them in temporary local files (one per processor). The resulting format is readable only on the same number of processors and with the same kind of parallelization used to write it.  Note that this flag has no effect on reading, only on writing.",
    )
    nstep: int | None = Field(
        None,
        description="number of molecular-dynamics or structural optimization steps performed in this run. If set to 0, the code performs a quick 'dry run', stopping just after initialization. This is useful to check for input correctness and to have the summary printed.",
    )
    ...
    lkpoint_dir: bool = Field(
        True,
        description="If .false. a subdirectory for each k_point is not opened in the 'prefix'.save directory; Kohn-Sham eigenvalues are stored instead in a single file for all k-points. Currently doesn't work together with wf_collect",
    )
...
class SystemNamelist(Namelist):
    ...
    lda_plus_u: bool = Field(
        False,
        description="Specify lda_plus_u = .TRUE. to enable DFT+U calculations See: Anisimov, Zaanen, and Andersen, PRB 44, 943 (1991) (https://journals.aps.org/prb/abstract/10.1103/PhysRevB.44.943); Anisimov et al., PRB 48, 16929 (1993) (https://journals.aps.org/prb/abstract/10.1103/PhysRevB.48.16929); Liechtenstein, Anisimov, and Zaanen, PRB 52, R5467 (1994) (https://journals.aps.org/prb/abstract/10.1103/PhysRevB.52.R5467). You must specify, for each species with a U term, the value of U and (optionally) alpha, J of the Hubbard model (all in eV): see lda_plus_u_kind, Hubbard_U, Hubbard_alpha, Hubbard_J",
    )
    lda_plus_u_kind: int = Field(
        0,
        description="Specifies the type of calculation:  0   DFT+U simplified version of Cococcioni and de Gironcoli, PRB 71, 035105 (2005) (https://journals.aps.org/prb/abstract/10.1103/PhysRevB.71.035105), using Hubbard_U  1   DFT+U rotationally invariant scheme of Liechtenstein et al., using Hubbard_U and Hubbard_J  2   DFT+U+V simplified version of Campo Jr and Cococcioni, J. Phys.: Condens. Matter 22, 055602 (2010), using Hubbard_V",
    )
    U_projection_type: Literal["atomic", "ortho-atomic", "norm-atomic", "file", "pseudo"] = Field(
        "atomic",
        description="Only active when lda_plus_U is .true., specifies the type of projector on localized orbital to be used in the DFT+U scheme.  Currently available choices:  NB: forces and stress currently implemented only for the 'atomic' and 'pseudo' choice.",
    )
    Hubbard_parameters: Literal["input", "file"] = Field("input", description="Available choices:")
    ...
    fcp_mu: float = Field(
        0.0e0,
        description="If lfcpopt = .TRUE., gives the target Fermi energy [Ry]. One can start with appropriate total charge of the system by giving 'tot_charge'.",
    )
    vdw_corr: Literal["none", "grimme-d2", "grimme-d3", "TS", "XDM"] = Field(
        "none",
    ...
    uniqueb: bool = Field(
        False,
        description="Used only for monoclinic lattices. If .TRUE. the b unique ibrav (-12 or -13) are used, and symmetry equivalent positions are chosen assuming that the two fold axis or the mirror normal is parallel to the b axis. If .FALSE. it is parallel to the c axis.",
    )
...
class ElectronsNamelist(Namelist):
    ...
    conv_thr: float = Field(
        1.0e-6,
        description="Convergence threshold for selfconsistency: estimated energy error < conv_thr (note that conv_thr is extensive, like the total energy).  For non-self-consistent calculations, conv_thr is used to set the default value of the threshold (ethr) for iterative diagonalizazion: see diago_thr_init",
    )
    ...
    diagonalization: Literal["david", "cg", "ppcg", "paro"] = Field(
...
class IonsNamelist(Namelist):
    ...
    ion_dynamics: Literal[
        None, "bfgs", "damp", "verlet", "langevin", "langevin-smc", "bfgs", "damp", "beeman"
    ] = Field(
        None,
