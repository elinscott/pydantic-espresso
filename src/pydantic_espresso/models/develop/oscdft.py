...
class OscdftNamelist(Namelist):
    ...
    iteration_type: Literal[None, 0, 1] = Field(None, description="Order of charge density and OS-CDFT multipliers optimizations.")
    optimization_method: Literal[None, None] = Field(None, description="Method to update the OS-CDFT multipliers.")
    ...
    max_conv_thr: float = Field(1.e-1, description="If iteration_type is 0, this is the starting convergence threshold. If iteration_type is 1, this is the convergence threshold. See iteration_type for more explanations.")
    min_conv_thr: float = Field(1.e-3, description="If iteration_type is 0, this is the minimum convergence threshold. If iteration_type is 1, this is ignored. See iteration_type for more explanations.")
    final_conv_thr: float = Field(-1.e0, description="If iteration_type is 0 and final_conv_thr > 0.D0, the charge density convergence is prevented when the OS-CDFT convergence test is larger than final_conv_thr. Otherwise, this is ignored.")
    conv_thr_multiplier: float = Field(0.5e0, description="If iteration_type is 0, see iteration_type for explanations. Otherwise, this is ignored.")
    ...
    min_gamma_n: float = Field(1.e0, description="Learning rate of optimizations. See optimization_method.")
    ...
    constraint_strength: float = Field(1.0e0, description="Multiplicative factor in front of the constraint term in the potential (and energy).")
    constraint_conv_thr: float = Field(5.0e-3, description="Convergence threshold for the mean absolute error (MAE) computed by averaging the absolute difference between the current and target occupation matrices. When this threshold is reached, the constarined is released.")
    ...
    constraint_mixing_beta: float = Field(4.0e-1, description="The multiplicative factor in front of the constraint term.")
