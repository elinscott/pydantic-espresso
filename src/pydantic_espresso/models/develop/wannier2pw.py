...
class InputppNamelist(Namelist):
    ...
    wan2hub: list[bool] | None = Field(
        ...
        description=(
            ...
            "number of selected Wannier functions must match the expected number of basis "
            "functions (e.g. 5 for d states, 3 for p states, etc per atom). In order to selected "
            "the Wannier functions, one has to inspect the output of Wannier90 calculations and "
            "see in which oredr the Wannier functions were generated."
        ),
    )
