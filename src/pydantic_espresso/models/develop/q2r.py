...
class InputNamelist(Namelist):
    ...
    fildyn: str | None = Field(
        ...
        description=(
            ...
            "the number of q-points in the irreducible brillouin zone.  Normally this should be "
            "the same as specified on input to the phonon code.  In the non collinear/spin-orbit "
            "case the files produced by ph.x are in .xml format. In this case fildyn is the same "
    ...
    zasr: Literal["no", "simple", "crystal", "one-dim", "zero-dim"] = Field(
        "no",
        description=(
            "Indicates the type of Acoustic Sum Rules used for the Born effective charges.  "
            "Allowed values:  Note that in certain cases, not all the rotational asr can be "
            "applied (e.g. if there are only 2 atoms in a molecule or if all the atoms are "
            "aligned, etc.). In these cases the supplementary asr are cancelled during the "
            "orthonormalization procedure (see below)."
        ),
    )
    loto_2d: bool = Field(
    ...
    write_lr: bool | None = Field(
        ...
        description=(
            ...
            "enforcing asr='all' for infrared- active solids in matdyn. An additional column will "
            "be written for long-range part of IFC for text format, while a tag named IFC_LR will "
            "be created for xml format."
        ),
    )
