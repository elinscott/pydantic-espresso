...
class InputNamelist(Namelist):
    ...
    noscf: bool = Field(
        False,
        description=(
            "if .true. the charge is not computed. The occupations are not used and the "
            "eigenvalues and eigenfunctions are those of a hydrogen-like atom."
        ),
    )
    lsd: int = Field(
        0,
    ...
    vdw: bool = Field(
        False,
        description=(
            "If .true., the frequency dependent polarizability and van der Waals coefficient C6 "
            "will be computed in Thomas-Fermi and von Weizsaecker approximation(only for "
            "closed-shell ions)."
        ),
    )
    prefix: str = Field(
        "ld1",
        description=(
            "Prefix for file names - only for output file names containing the orbitals, "
            "logarithmic derivatives, tests See below for file names and the content of the file."
        ),
    )
    verbosity: Literal["low", "high"] = Field(
        "low", description="Controls how much information is printed on output."
    )
    ...
    config: str | None = Field(
        ...
        description=(
            ...
            "lsd=1, spin-up and spin-down state may appear twice with the respective occupancy: "
            "3p4 3p2 = 4 up, 2 down. Otherwise, the Hund's rule is assumed.  * If rel=2, states "
            "with jj=l-1/2 are filled first. If a state appears twice, the first one has jj=l-1/2, "
            "the second one jj=l+1/2 (except S states) (Use rel_dist if you want to average the "
            "electrons over all available states.)  * If config='default' the code uses zed to set "
            "the ground state electronic configuration for the atom.  Negative occupancies are "
            "used to flag unbound states; they are not actually used."
        ),
    )
    relpert: bool = Field(
        False,
        description=(
            "If .true. the relativistic corrections to the non-relativistic Kohn-Sham energy "
            "levels (rel=0 .and. lsd=0) are computed using first-order perturbation theory in "
            "all-electron calculations. The corrections consist of the following terms: E_vel: "
            "velocity (p^4) correction E_Dar: Darwin term E_S-O: spin-orbit coupling The "
            "spin-orbit term vanishes for s-electron states and gives rise to a splitting of "
            "(2*l+1)*E_S-O for the other states. The separate contributions are printed only if "
            "verbosity='high'.  Formulas and notation are based on the Herman-Skillman book: F. "
            "Herman and S. Skillman, 'Atomic Structure Calculations', Prentice-Hall, Inc., "
            "Englewood Cliffs, New Jersey, 1963"
        ),
    )
    rel_dist: Literal["energy", "average"] = Field(
        "energy",
        description="Distribution of electrons among the relativistic states with the same l.",
    )
    write_coulomb: bool = Field(
        False,
        description=(
            "If .true., a fake pseudo-potential file with name X.UPF, where X is the atomic "
            "symbol, is written. It contains the radial grid and the wavefunctions as specified in "
            "input, plus the info needed to build the Coulomb potential for an all-electron "
            "calculation - for testing only."
        ),
    )
...
class InputpNamelist(Namelist):
    ...
    zval: Annotated[float | None, Quantity(units="e", dimensionality="charge")] = Field(
        ...
        json_schema_extra={"computed_default": True},
        description=(
            "Valence charge.  zval is automatically calculated from available data. If the value "
            "of zval is provided in input, it will be checked versus the calculated value. The "
            "only case in which you need to explicitly provide the value of zval for noninteger "
            "zval (i.e. half core-hole pseudo-potentials)."
        ),
    )
    pseudotype: int = Field(
        0,
        description=(
            "1 ... norm-conserving, single-projector PP IMPORTANT: if pseudotype=1 all "
            "calculations are done using the SEMILOCAL form, not the separable nonlocal form  2 "
            "... norm-conserving PP in separable form (obsolescent) All calculations are done "
            "using SEPARABLE non-local form IMPORTANT: multiple projectors allowed but not "
            "properly implemented, use only if you know what you are doing  3 ... ultrasoft PP or "
            "PAW"
        ),
    )
    file_pseudopw: str | None = Field(
        ...
        description=(
            ...
            "or in any case for spin-orbit PP (rel=2), the file is written in UPF format;  * if "
            "the file name ends with 'psp' it is written in native CPMD format (this is currently "
            "an experimental feature); otherwise it is written in the old 'NC' format if "
            "pseudotype=1, or in the old RRKJ format if pseudotype=2 or 3 (no default, must be "
            "specified)."
        ),
    )
    file_recon: str | None = Field(
        None,
        description=(
            "File containing data needed for GIPAW reconstruction of all-electron wavefunctions "
            "from PP results. If you want to use additional states to perform the reconstruction, "
            "add them at the end of the list of all-electron states."
        ),
    )
    lloc: int = Field(
        -1,
        description=(
            "Angular momentum of the local channel.  * lloc=-1 or lloc=-2 pseudizes the "
            "all-electron potential if lloc=-2 the original recipe of Troullier-Martins is used "
            "(zero first and second derivatives at r=0) * lloc>-1 uses the corresponding channel "
            "as local PP  NB: if lloc>-1, the corresponding channel must be the last in the list "
            "of wavefunctions appearing after the namelist &inputp In the relativistic case, if "
            "lloc > 0 both the j=lloc-1/2 and the j=lloc+1/2 wavefunctions must be at the end of "
            "the list."
        ),
    )
    ...
    nlcc: bool = Field(
        False,
        description=(
            "If .true. produce a PP with the nonlinear core correction of Louie, Froyen, and Cohen "
            "[PRB 26, 1738 (1982) "
    ...
    tm: bool = Field(
        ...
        description=(
            ...
            "(https://journals.aps.org/prb/abstract/10.1103/PhysRevB.41.1227), erratum PRB 44, "
            "13175 (1991) (https://journals.aps.org/prb/abstract/10.1103/PhysRevB.44.13175)]"
        ),
    )
    rho0: Annotated[float, Quantity(units="e bohr^-3", dimensionality="charge length^-3")] = Field(
        ...
        description=(
            ...
            "functions fails, specifying rho0 > 0 may allow to override the problem (using 4 "
            "Bessel functions). Typical values are in the order of 0.01-0.02"
        ),
    )
    lpaw: bool = Field(
        False,
    ...
    lsave_wfc: bool | None = Field(
        ...
        description=(
            ...
            "pseudo-potential generation in the UPF file. Only works for UPFv2 format.  The "
            "default is .false. for a norm-conserving or ultrasoft calculation (.not. lpaw), and "
            ".true. for a PAW calculation (lpaw=.true.)."
        ),
    )
    lgipaw_reconstruction: bool = Field(
        False,
        description=(
            "Set it to .true. to generate pseudo-potentials containing the additional info "
            "required for reconstruction of all-electron orbitals, used by GIPAW. You will "
            "typically need to specify additional projectors beyond those used in the generation "
            "of pseudo-potentials. You should also specify file_recon.  All projectors used in the "
            "reconstruction must be listed BOTH in the test configuration after namelist &test AND "
            "in the all-electron configuration (variable 'config', namelist &inputp, Use negative "
            "occupancies for projectors on unbound states). The core radii in the test "
            "configuration should be the same as in the pseudo-potential generation section and "
            "will be used as the radius of reconstruction. Projectors not used to generate the "
    ...
    use_paw_as_gipaw: bool = Field(
        ...
        description=(
            ...
            "using the PAW data and projectors for the valence wavefunctions.  In the default "
            "case, the GIPAW valence wavefunction and projectors are independent from the PAW ones "
            "and must be then specified as explained above in lgipaw_reconstruction.  Setting this "
            "to .true. always implies lgipaw_reconstruction = .true."
        ),
    )
...
class TestNamelist(Namelist):
    ...
    file_pseudo: str | None = Field(
        ...
        description=(
            ...
            "not the separable nonlocal form. Use the UPF format if you want to test the separable "
            "form!"
        ),
    )
    ...
    frozen_core: bool = Field(
        ...
        description=(
            ...
            "eigenvalues, orbitals and energies of the other configurations are calculated with "
            "the core of the first configuration. The first configuration must be spin-unpolarized."
        ),
    )
    ...
    configts: list[str] | None = Field(
        None,
        description=(
            "A string array containing the test electronic configuration. configts(nc), "
            "nc=1,nconf, has the same syntax as for config but only VALENCE states must be "
            "included. If configts(i) is not set, the electron configuration is read from the "
            "cards following the namelist. (start = 1, end = nconf)"
        ),
    )
