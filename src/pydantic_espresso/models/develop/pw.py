...
class ControlNamelist(Namelist):
    ...
    nstep: int | None = Field(
        ...
        description=(
            "number of molecular-dynamics or structural optimization steps performed in this run. "
            "If set to 0, the code performs a quick 'dry run', stopping just after initialization. "
            "This is useful to check for input correctness and to have the summary printed. NOTE: "
            "in MD calculations, the code will perform 'nstep' steps even if restarting from a "
    ...
    wfcdir: Path | None = Field(
        ...
        description=(
            ...
            "local file system, while outdir should be a parallel or network file system, visible "
            "to all processors. Beware: in order to restart from interrupted runs, or to perform "
    ...
    max_seconds: Annotated[float, Quantity(units="s", dimensionality="time")] = Field(
        ...
        description=(
            ...
            "restart_mode if you need to split a job too long to complete into shorter jobs that "
            "fit into your batch queues. The default value (10000000 s, about 150 days) "
            "effectively means that no time limit is imposed."
        ),
    )
    etot_conv_thr: Annotated[float, Quantity(units="Ry", dimensionality="energy")] = Field(
        ...
        description=(
            ...
            "two consecutive scf steps. Note that etot_conv_thr is extensive, like the total "
            "energy. See also forc_conv_thr - both criteria must be satisfied"
        ),
    )
    ...
    disk_io: Literal["default", "high", "medium", "low", "nowf", "minimal", "none"] = Field(
        "default",
        description=(
            "Specifies the amount of disk I/O activity: (only for binary files and xml data file "
            "in data directory; other files printed at each molecular dynamics / structural "
    ...
    tefield: bool = Field(
        False,
        description=(
            "If .TRUE. a saw-like potential simulating an electric field is added to the bare "
            "ionic potential. See variables edir, eamp, emaxpos, eopreg for the form and size of "
            "the added potential."
        ),
    )
    dipfield: bool = Field(
        ...
        description=(
            ...
            "(https://journals.aps.org/prb/abstract/10.1103/PhysRevB.59.12301). See variables "
            "edir, emaxpos, eopreg for the form of the correction. Must be used ONLY in a slab "
            "geometry, for surface calculations, with the discontinuity FALLING IN THE EMPTY SPACE."
        ),
    )
    lelfield: bool = Field(
        False,
    ...
    nberrycyc: int = Field(
        1,
        description=(
            "In the case of a finite electric field  ( lelfield == .TRUE. ) it defines the number "
            "of iterations for converging the wavefunctions in the electric field Hamiltonian, for "
            "each external iteration on the charge density"
        ),
    )
    lorbm: bool = Field(
        False,
        description=(
            ...
            "and should be performed on an automatically generated uniform grid of k points.  "
    ...
    gdir: int = Field(
        ...
        description=(
            ...
            "calculations with finite electric fields (lelfield==.true.) 'gdir' is the direction "
            "of the field."
        ),
    )
    nppstr: int = Field(
    ...
    gate: bool = Field(
        ...
        description=(
            ...
            "charged plate, which is placed at zgate (see below). Details of the gate potential "
            "can be found in T. Brumme, M. Calandra, F. Mauri; PRB 89, 245406 (2014) "
            ...
            "dipole correction! (dipfield=.true.). Currently, symmetry can be used with "
            "gate=.true. but carefully check that no symmetry is included which maps z to -z even "
            "if in principle one could still use them for symmetric systems (i.e. no dipole "
            "correction). For nosym=.false. verbosity is set to 'high'. Note: this option was "
            "called 'monopole' in v6.0 and 6.1 of pw.x"
        ),
    )
    twochem: bool = Field(
        False,
    ...
    lfcp: bool = Field(
        False,
        description=(
            "If .TRUE. perform a constant bias potential (constant-mu) calculation for a system "
            "with ESM method. See the header of PW/src/fcp_module.f90 for documentation. To "
            "perform the calculation, you must set a namelist FCP.  NB: - The total energy "
            "displayed in output includes the potentiostat contribution (-mu*N). - calculation "
            "must be 'relax' or 'md'. - assume_isolated = 'esm' and esm_bc = 'bc2' or 'bc3' must "
            "be set in SYSTEM namelist. - ESM-RISM is also supported (assume_isolated = 'esm' and "
            "esm_bc = 'bc1' and trism = .TRUE.). - ignore_wolfe is always .TRUE., for BFGS."
        ),
    )
    trism: bool = Field(
        False,
        description=(
            ...
            "assume_isolated = 'esm' and esm_bc = 'bc1', Laue-RISM is calculated instead of "
            "3D-RISM and coupled with ESM method (i.e. ESM-RISM). [for details see S.Nishihara and "
            "M.Otani, PRB 96, 115429 (2017) "
...
class SystemNamelist(Namelist):
    ...
    nbnd: int | None = Field(
        ...
        description=(
            ...
            "By default, for an insulator nbnd is set to the number of valence bands (nbnd = "
            "number of electrons / 2); for a metal, 20% more (with a minimum of 4 more) states are "
            "added."
        ),
    )
    nbnd_cond: int | None = Field(
        ...
        description=(
            ...
            "calculation (twochem=.true.).  By default it is set to nbnd - (number of electrons / "
            "2) in the collinear case, and to nbnd - (number of electrons) in the noncollinear "
            "case."
        ),
    )
    ...
    tot_magnetization: float = Field(
        ...
        description=(
            ...
            "electronic magnetization. The default value -10000 is a sentinel meaning "
            "'unspecified': if left unspecified the tot_magnetization variable is ignored and the "
            "amount of electronic magnetization is determined during the self-consistent cycle."
        ),
    )
    ...
    ecutfock: Annotated[float | None, Quantity(units="Ry", dimensionality="energy")] = Field(
        ...
        description=(
            ...
            "speed-up can be obtained by reducing ecutfock, at the expense of some loss in "
            "accuracy. Must be .gt. ecutwfc. Not implemented for stress calculation and for US-PP "
            "and PAW pseudopotentials. Use with care, especially in metals where it may give raise "
            "to instabilities."
        ),
    )
    nosym: bool = Field(
        False,
        description=(
            "if (.TRUE.) symmetry is not used. Consequences:  - if a list of k points is provided "
            "in input, it is used 'as is': symmetry-inequivalent k-points are not generated, and "
            "the charge density is not symmetrized;  - if a uniform (Monkhorst-Pack) k-point grid "
            "is provided in input, it is expanded to cover the entire Brillouin Zone, irrespective "
            "of the crystal symmetry. Time reversal symmetry is assumed so k and -k are considered "
            "as equivalent unless noinv=.true. is specified.  Do not use this option unless you "
            "know exactly what you want and what you get. May be useful in the following cases: - "
            "in low-symmetry large cells, if you cannot afford a k-point grid with the correct "
            "symmetry - in MD simulations - in calculations for isolated atoms"
        ),
    )
    nosym_evc: bool = Field(
        ...
        description=(
            ...
            "Bravais lattice; an automatically generated Monkhorst-Pack grid will contain all "
            "points of the grid over the entire Brillouin Zone, plus the points rotated by the "
            "symmetries of the Bravais lattice which were not in the original grid. The same "
            "applies if a k-point list is provided in input instead of a Monkhorst-Pack grid. Time "
            "reversal symmetry is assumed so k and -k are equivalent unless noinv=.true. is "
            "specified. This option differs from nosym because it forces k-points in all cases to "
            "have the full symmetry of the Bravais lattice (not all uniform grids have such "
            "property!)"
        ),
    )
    noinv: bool = Field(
    ...
    force_symmorphic: bool = Field(
        False,
        description=(
            "if (.TRUE.) force the symmetry group to be symmorphic by disabling symmetry "
            "operations having an associated fractionary translation"
        ),
    )
    use_all_frac: bool = Field(
        ...
        description=(
            ...
            "in real space that works for non-symmorphic operations only if the real-space FFT "
            "grids are commensurate."
        ),
    )
    occupations: Literal[
        "smearing", "tetrahedra", "tetrahedra_lin", "tetrahedra_opt", "fixed", "from_input"
    ] = Field("fixed", description="Available options are:")
    one_atom_occupations: bool = Field(
        False,
        description=(
            "This flag is used for isolated atoms (nat=1) together with occupations='from_input'. "
            "If it is .TRUE., the wavefunctions are ordered as the atomic starting wavefunctions, "
            "independently from their eigenvalue. The occupations indicate which atomic states are "
            "filled.  The order of the states is written inside the UPF pseudopotential file. In "
            "the scalar relativistic case: S -> l=0, m=0 P -> l=1, z, x, y D -> l=2, r^2-3z^2, xz, "
            "yz, xy, x^2-y^2  In the noncollinear magnetic case (with or without spin-orbit), each "
            "group of states is doubled. For instance: P -> l=1, z, x, y for spin up, l=1, z, x, y "
            "for spin down. Up and down is relative to the direction of the starting "
            "magnetization.  In the case with spin-orbit and time-reversal "
            "(starting_magnetization=0.0) the atomic wavefunctions are radial functions multiplied "
            "by spin-angle functions. For instance: P -> l=1, j=1/2, m_j=-1/2,1/2. l=1, j=3/2, "
            "m_j=-3/2, -1/2, 1/2, 3/2.  In the magnetic case with spin-orbit the atomic "
            "wavefunctions can be forced to be spin-angle functions by setting starting_spin_angle "
            "to .TRUE.."
        ),
    )
    starting_spin_angle: bool = Field(
        ...
        description=(
            ...
            "initialized as in scalar relativistic noncollinear case without spin-orbit.  By "
            "setting starting_spin_angle=.TRUE. this behaviour can be changed and the initial "
            "wavefunctions are radial functions multiplied by spin-angle functions.  When "
            "domag=.FALSE. the initial wavefunctions are always radial functions multiplied by "
            "spin-angle functions independently from this flag.  When lspinorb is .FALSE. this "
            "flag is not used."
        ),
    )
    ...
    nelec_cond: float = Field(
        ...
        description=(
            ...
            "calculation (twochem=.true.). Of the total # of electrons nelec, nelec-nelec_cond "
            "will occupy the valence manifold and nelec_cond will be constrained in the conduction "
            "manifold."
        ),
    )
    ...
    sic_energy: bool = Field(
        ...
        description=(
            ...
            "polaron. When .false., the total energy printed in output should not be considered. "
            "For structural relaxations, it is recommended to use .false. to avoid doubling the "
            "computational cost."
        ),
    )
    ...
    ace: bool = Field(
        ...
        description=(
            ...
            "(https://doi.org/10.1021/acs.jctc.6b00092)  Set to false to use standard Exchange "
            "(much slower)"
        ),
    )
    ...
    screening_parameter: Annotated[float, Quantity(units="bohr^-1", dimensionality="length^-1")] = (
        Field(
            0.106,
            description=(
                "screening_parameter for HSE like hybrid functionals. For more information, see: "
    ...
    localization_thr: float = Field(
        ...
        description=(
            ...
            "the SCDM localization and the evaluation on EXX using the localized orbitals. Very "
            "small value of the threshold should yield the same result as the default EXX "
            "evaluation"
        ),
    )
    dmft: bool = Field(
        False,
        description=(
            "If true, nscf calculation will exit in restart mode, scf calculation will restart "
            "from there if DMFT updates are provided as hdf5 archive. Scf calculation should be "
            "used only with electron_maxstep = 1. K_POINTS have to be identical and given "
            "explicitly with nosym."
        ),
    )
    dmft_prefix: str | None = Field(
        ...
        description=(
            ...
            "group/dataset as: - dft_misc_input/band_window with dimension [1, number of k-points, "
            "2 (real + complex)] - dft_update/delta_N with dimension [number of k-points, number "
            "of correlated orbitals, number of correlated orbitals, 2 (real + complex)]"
        ),
    )
    ensemble_energies: bool = Field(
        ...
        description=(
            ...
            "correlation ratios after each converged electronic ground state calculation.  "
            "Ensemble energies can be analyzed with the 'bee' utility included with libbeef.  "
            "Requires linking against libbeef. input_dft must be set to a BEEF-type functional "
    ...
    edir: int = Field(
        1,
        description=(
            "The direction of the electric field or dipole correction is parallel to the "
            "bg(:,edir) reciprocal lattice vector, so the potential is constant in planes defined "
            "by FFT grid points; edir = 1, 2 or 3. Used only if tefield is .TRUE."
        ),
    )
    emaxpos: float = Field(
        0.5,
    ...
    eamp: Annotated[
        ...
        0.0,
        description=(
            "Amplitude of the electric field, in ***Hartree*** a.u.; 1 a.u. = 51.4220632*10^10 "
            "V/m. Used only if tefield==.TRUE. The saw-like potential increases with slope eamp in "
            "the region from (emaxpos+eopreg-1) to (emaxpos), then decreases to 0 until "
    ...
    Lambda: float = Field(
        1.0,
        description=(
            "parameter used for constrained_magnetization calculations N.B.: if the scf "
            "calculation does not converge, try to reduce lambda to obtain convergence, then "
            "restart the run with a larger lambda"
        ),
    )
    report: int = Field(
        -1,
        description=(
            "determines when atomic magnetic moments are printed on output: report = 0  never "
            "report =-1  at the beginning of the scf and at convergence report = N  as -1, plus "
            "every N scf iterations"
        ),
    )
    lspinorb: bool = Field(
        False,
    ...
    esm_bc: Literal["pbc", "bc1", "bc2", "bc3"] = Field(
        "pbc",
        description=(
            "If assume_isolated = 'esm', determines the boundary conditions used for either side "
            "of the slab.  Currently available choices:"
        ),
    )
    esm_w: Annotated[float, Quantity(units="bohr", dimensionality="length")] = Field(
        ...
        description=(
            ...
            "effective screening region, measured relative to the cell edge. (ESM region begins at "
            "z = +/- [L_z/2 + esm_w] )."
        ),
    )
    ...
    lgcscf: bool = Field(
        ...
        description=(
            ...
            "Grand-Canonical SCF. (JCP 146, 114104 (2017), R.Sundararaman, et al.)  NB: - The "
            "total energy displayed in output includes the potentiostat contribution (-mu*N). - "
            ...
            "diago_thr_init is 1.D-5. - diago_full_acc is always .TRUE. . - diago_rmm_conv is "
            "always .TRUE. ."
        ),
    )
    ...
    xdm_a1: float | None = Field(
        ...
        description=(
            ...
            "the functional is one of B86bPBE, PW86PBE, PBE, BLYP. For functionals in this list, "
            "the coefficients are given in: "
            "https://github.com/aoterodelaroza/postg/blob/master/xdm.param or "
            "https://erin-r-johnson.github.io/software/ A. Otero de la Roza, E. R. Johnson, J. "
            "Chem. Phys. 138, 204109 (2013), doi:10.1063/1.4705760 "
    ...
    xdm_a2: Annotated[float | None, Quantity(units="angstrom", dimensionality="length")] = Field(
        ...
        json_schema_extra={"computed_default": True},
        description=(
            "Damping function parameter a2. It is NOT necessary to give a value if the functional "
            "is one of B86bPBE, PW86PBE, PBE, BLYP. For functionals in this list, the coefficients "
            "are given in: https://github.com/aoterodelaroza/postg/blob/master/xdm.param or "
            "https://erin-r-johnson.github.io/software/ A. Otero de la Roza, E. R. Johnson, J. "
            "Chem. Phys. 138, 204109 (2013), doi:10.1063/1.4705760 "
            "(https://doi.org/10.1063/1.4705760)"
        ),
    ...
    uniqueb: bool = Field(
        False,
        description=(
            "Used only for monoclinic lattices. If .TRUE. the b unique ibrav (-12 or -13) are "
            "used, and symmetry equivalent positions are chosen assuming that the twofold axis or "
    ...
    starting_magnetization: list[float] = Field(
        ...
        description=(
            ...
            "Alternatively, the values can range between -1 and 1, which will be interpreted as "
            "the site magnetization per valence electron. For QE-v7.2 and older versions, only the "
            "second option is allowed.  If you expect a nonzero magnetization in your ground "
            "state, you MUST either specify a nonzero value for at least one atomic type, or "
            "constrain the magnetization using variable tot_magnetization for LSDA, "
            "constrained_magnetization for noncollinear/spin-orbit calculations. If you don't, you "
            "will get a nonmagnetic (zero magnetization) state. In order to perform LSDA "
            "calculations for an antiferromagnetic state, define two different atomic species "
            "corresponding to sublattices of the same atomic type.  NOTE 1: starting_magnetization "
            "is ignored in most BUT NOT ALL cases in non-scf calculations: it is safe to keep the "
            "same values for the scf and subsequent non-scf calculation.  NOTE 2: If you fix the "
            "magnetization with tot_magnetization, do not specify starting_magnetization.  NOTE 3: "
            "In the noncollinear/spin-orbit case, starting with zero starting_magnetization on all "
            "atoms imposes time reversal symmetry. The magnetization is never calculated and is "
            "set to zero (the internal variable domag is set to .FALSE.). (start = 1, end = ntyp)"
        ),
    )
...
class ElectronsNamelist(Namelist):
    ...
    electron_maxstep: int = Field(
        ...
        description=(
            ...
            "interrupted calculation: maximum number of scf iterations performed in the current "
            "run, irrespective of how many have already been performed in previous runs."
        ),
    )
    ...
    scf_must_converge: bool = Field(
        True,
        description=(
            "If .false. do not stop molecular dynamics or ionic relaxation when electron_maxstep "
            "is reached. Use with care."
        ),
    )
    conv_thr: Annotated[float, Quantity(units="Ry", dimensionality="energy")] = Field(
        1e-06,
        description=(
            "Convergence threshold for selfconsistency: estimated energy error < conv_thr (note "
            "that conv_thr is extensive, like the total energy).  For non-self-consistent "
            "calculations, conv_thr is used to set the default value of the threshold (ethr) for "
    ...
    mixing_fixed_ns: int = Field(
        0,
        description=(
            "For DFT+U : number of iterations with fixed ns ( ns is the atomic density appearing "
            "in the Hubbard term )."
        ),
    )
    diagonalization: Literal[
        "david", "cg", "ppcg", "paro", "rmm-davidson", "rmm-paro", "direct"
    ] = Field("david", description="Available options are:")
    diago_thr_init: float = Field(
        0.0,
        description=(
            "Convergence threshold (ethr) for iterative diagonalization (the check is on "
            "eigenvalue convergence).  For scf calculations: default is 1.D-2 if starting from a "
            "superposition of atomic orbitals; 1.D-5 if starting from a charge density. During "
            "self consistency the threshold is automatically reduced (but never below 1.D-13) when "
            "approaching convergence.  For non-scf calculations: default is (conv_thr/N elec)/10."
        ),
    )
    ...
    diago_david_ndim: int = Field(
        2,
        description=(
            "For Davidson diagonalization: dimension of workspace (number of wavefunction packets, "
            "at least 2 needed). A larger value may yield a smaller number of iterations in the "
            "algorithm but uses more memory and more CPU time in subspace diagonalization "
    ...
    diago_rmm_conv: bool = Field(
        False,
        description=(
            "If .TRUE., RMM-DIIS is performed up to converge. If .FALSE., RMM-DIIS is performed "
            "only once."
        ),
    )
    ...
    startingpot: Literal["atomic", "file"] = Field("atomic", description="Available options are:")
    ...
    real_space: bool = Field(
        ...
        description=(
            ...
            "projectors. Faster and in principle better scaling than the default G-space "
            "algorithm, but numerically less accurate, may lead to some loss of translational "
...
class IonsNamelist(Namelist):
    ...
    pot_extrapolation: Literal["none", "atomic", "first_order", "second_order"] = Field(
        "atomic",
        description=(
            "Used to extrapolate the potential from preceding ionic steps.  Note: 'first_order' "
    ...
    wfc_extrapolation: Literal["none", "first_order", "second_order"] = Field(
        "none",
        description=(
            "Used to extrapolate the wavefunctions from preceding ionic steps.  Note: "
            "'first_order' and 'second-order' extrapolation make sense only for molecular dynamics "
            "calculations"
        ),
    )
...
class CellNamelist(Namelist):
    ...
    cell_dynamics: Literal["none", "sd", "damp-pr", "damp-w", "bfgs", "none", "pr", "w"] = Field(
        "none",
        description=(
            "Specify the type of dynamics for the cell. For different type of calculation "
            "different possibilities are allowed and different default values apply:  CASE ( "
            "calculation == 'vc-relax' )  CASE ( calculation == 'vc-md' )"
        ),
    )
    ...
    wmass: Annotated[float | None, Quantity(units="amu", dimensionality="mass")] = Field(
        ...
        description=(
            ...
            "0.75*Tot_Mass/pi**2 for Parrinello-Rahman MD, and as 0.75*Tot_Mass/pi**2/Omega**(2/3) "
            "for Wentzcovitch MD."
        ),
    )
...
class FcpNamelist(Namelist):
    ...
    fcp_mu: Annotated[float | None, Quantity(units="eV", dimensionality="energy")] = Field(
        None,
        description=(
    ...
    fcp_dynamics: Literal["none", "bfgs", "newton", "damp", "lm", "velocity-verlet", "verlet"] = (
        Field(
            ...
            description=(
                ...
                "default values apply:  CASE ( calculation == 'relax' )  CASE ( calculation == "
                "'md' )"
            ),
        )
    )
...
class RismNamelist(Namelist):
    ...
    ecutsolv: Annotated[float | None, Quantity(units="Ry", dimensionality="energy")] = Field(
        ...
        description=(
            ...
            "solute (e.g. Zeolite, MOF), it is desirable to apply a larger value. Solvents "
            "confined in a framework often have a high frequency."
        ),
    )
    starting1d: Literal["zero", "file", "fix"] = Field("zero", description="")
    starting3d: Literal["zero", "file"] = Field("zero", description="")
    ...
    rism1d_dielectric: float = Field(
        ...
        description=(
            ...
            "RISM (DRISM) is performed.  For details of DRISM, see: J.S.Perkyns and B.M.Pettitt, "
            "CPL 1992, 190, 626, doi:10.1016/0009-2614(92)85201-K "
    ...
    laue_expand_right: Annotated[float, Quantity(units="bohr", dimensionality="length")] = Field(
        ...
        description=(
            ...
            "right-hand side of the unit cell, measured relative to the unit cell edge. (the "
            "solvent region ends at z = + [L_z/2 + laue_expand_right].) This is only for Laue-RISM."
        ),
    )
    laue_expand_left: Annotated[float, Quantity(units="bohr", dimensionality="length")] = Field(
        ...
        description=(
            "If positive value, set the ending position offset [in a.u.] of the solvent region on "
            "left-hand side of the unit cell, measured relative to the unit cell edge. (the "
            "solvent region ends at z = - [L_z/2 + laue_expand_left].) This is only for Laue-RISM."
        ),
    )
    ...
    laue_starting_left: Annotated[float, Quantity(units="bohr", dimensionality="length")] = Field(
        0.0,
        description=(
            "Set the starting position [in a.u.] of the solvent region on left-hand side of the "
            "unit cell. Then the solvent region is defined as [ -L_z/2 - laue_expand_left , "
    ...
    laue_buffer_right: Annotated[float | None, Quantity(units="bohr", dimensionality="length")] = (
        Field(
            ...
            description=(
                ...
                "right-hand side of the unit cell. Then correlation functions are defined inside "
                "of [ laue_starting_right - laue_buffer_right , L_z/2 + laue_expand_right ]. This "
                "is only for Laue-RISM."
            ),
        )
    )
    laue_buffer_left: Annotated[float | None, Quantity(units="bohr", dimensionality="length")] = (
        Field(
            ...
            description=(
                ...
                "[ -L_z/2 - laue_expand_left , laue_starting_left + laue_buffer_left ]. This is "
                "only for Laue-RISM."
            ),
        )
    )
    ...
    laue_wall: Literal["none", "auto", "manual"] = Field(
        "auto",
        description=(
            "Set the repulsive wall with (1/r)^12 term of Lennard-Jones potential. This is only "
            "for Laue-RISM."
        ),
    )
    laue_wall_z: Annotated[float, Quantity(units="bohr", dimensionality="length")] = Field(
        0.0,
        description=(
            "Set the edge position [in a.u.] of the repulsive wall. If laue_expand_right > 0.0, "
            "the repulsive wall is defined on [ -inf , laue_wall_z ]. If laue_expand_left > 0.0, "
            "the repulsive wall is defined on [ laue_wall_z , inf ]. This is only for Laue-RISM "
            "and laue_wall == 'manual' ."
        ),
    )
    laue_wall_rho: Annotated[float, Quantity(units="bohr^-3", dimensionality="length^-3")] = Field(
        0.01,
        description=(
            "The density (1/bohr^3) of the repulsive wall. This is only for Laue-RISM and "
            "laue_wall /= 'none' ."
        ),
    )
    laue_wall_epsilon: Annotated[float, Quantity(units="kcal mol^-1", dimensionality="energy")] = (
        Field(
            0.1,
            description=(
                "The Lennard-Jones potential of the repulsive wall. Here, you can set the "
                "parameter 'epsilon'. This is only for Laue-RISM and laue_wall /= 'none' ."
            ),
        )
    )
