...
class ControlNamelist(Namelist):
    ...
    outdir: Path | None = Field(
        ...
        description=(
            ...
            "in the calculation of previous PW calculation."
        ),
    )
    ...
    kcw_at_ks: bool = Field(
        True,
        description=(
            "If true the KS canonical orbitals are used instead of Wannier functions. It makes "
            "sense for isolated system only."
        ),
    )
    ...
    homo_only: bool = Field(
        False,
        description=(
            "If kcw_at_ks = .TRUE. only the screening paramenter for the HOMO is calculated. "
            "Mainly for a perturbative calculation of the first Ionization Potential in isolated "
            "systems."
        ),
    )
    l_vcut: bool = Field(
        ...
        description=(
            ...
            "Coulomb integral (bare and screened). Improves the convergence wrt k/q-point "
            "sampling. Requires to correctly set eps_inf for the calculation of the screened "
            "interaction.  Use it only for periodic system. For isoleted system use "
            "assume_isolated, instead."
        ),
    )
    ...
    irr_bz: bool = Field(
        False,
        description=(
            "If .true. compute and use the symmetries of the Wannier orbital densities to reduce "
            "the number of k and q points for the BZ sampling."
        ),
    )
    use_wct: bool = Field(
        False,
...
class WannierNamelist(Namelist):
    ...
    seedname: str | None = Field(
        ...
        description=(
            "The seedname of the previous Wannier90 calculation for occupied states. NOTA BENE: "
            "the code implicitely assumed that the seedname for empty state is the same as that "
            "for occupied state with '_emp' appended. Keep this in mind when set up the wannier90 "
    ...
    check_ks: bool = Field(
        False,
        description=(
            "Specify if a diagonalization of the KS matrix build using the wannier function in "
            "input has to be performed. This is mainly for debugging purpose."
        ),
    )
...
class ScreenNamelist(Namelist):
    ...
    eps_inf: float = Field(
        ...
        description=(
            ...
            "equivalent to a Makov-Payne correction. It works well for cubic systems. Less well "
            "for anisotropic systems.  ANISOTROPIC SYSTEMS: In this case a generalization of the "
            "GB scheme is implemented based on Nano Lett.,9, 975 (2009). It requires the full "
            ...
            "the value eps_inf provided in input will be used (isotropic approximation). If not "
            "even eps_inf is provided in input no correction is applied to the screened KC "
            "correction."
        ),
    )
    check_spread: bool = Field(
        ...
        description=(
            ...
            "decide whether two or more Wannier functions can be considered 'identical' or not. "
            "Two Wannier functions are considered identical if their spread (self-hartree) differ "
            "by less than 1e-4 Ry (Hard coded for now, see spread_thr)."
        ),
    )
...
class HamNamelist(Namelist):
    ...
    use_ws_distance: bool = Field(
        ...
        description=(
            ...
            "proper distance and to have a smoother interpolation. Requires seedname_centres.xyz "
            "to be printed by the previous Wannier90 run. If the file is not found it is "
            "automatically switched to .FALSE. and only the distance between the cells is used "
            "(see also Wannier90 documentation)"
        ),
    )
    write_hr: bool = Field(
        True,
