...
class ControlNamelist(Namelist):
    ...
    verbosity: Literal["debug", "high", "medium", "low", "minimal"] = Field(
        "low", description="In order of decreasing verbose output:"
    )
    ...
    iprint: int | None = Field(
        ...
        description=(
            ...
            "named as 'prefix.???' depending on 'prefix' parameter. In the standard output "
            "relevant quantities are written every 10*iprint steps."
        ),
    )
    tstress: bool = Field(
        False,
    ...
    prefix: str | None = Field(
        ...
        description=(
            "prepended to input/output filenames and restart folders: prefix.pos : atomic "
            "positions prefix.vel : atomic velocities prefix.for : atomic forces prefix.cel : cell "
            "parameters prefix.str : stress tensors prefix.evp : energies prefix.hrs : Hirshfeld "
            "effective volumes (ts-vdw) prefix.eig : eigen values prefix.nos : Nose-Hoover "
            "variables prefix.spr : spread of Wannier orbitals prefix.wfc : center of Wannier "
            "orbitals prefix.ncg : number of Poisson CG steps (PBE0) prefix_ndw.save/ : write "
            "restart folder prefix_ndr.save/ : read restart folder where ndr and ndw are the "
            "integers number described below"
        ),
    )
    ndr: int = Field(
        50,
        description=(
            "The restart files are read from the folder outdir/prefix_ndr.save/ where outdir, "
            "prefix and ndr are the input variables described in this document"
        ),
    )
    ndw: int = Field(
        50,
        description=(
            "The restart files are written, if ndw > 0, in the folder outdir/prefix_ndw.save/ "
            "where outdir, prefix and ndw are the input variables described in this document"
        ),
    )
    tabps: bool = Field(
        False,
        description=(
            ...
            "(https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.94.145501); JCP 124, "
            "074103 (2006))."
        ),
    )
    ...
    etot_conv_thr: Annotated[float, Quantity(units="Hartree", dimensionality="energy")] = Field(
        1.0e-4,
        description=(
            "convergence threshold on total energy for ionic minimization: the convergence "
            "criterion is satisfied when the total energy changes less than etot_conv_thr between "
            "two consecutive scf steps. See also forc_conv_thr - both criteria must be satisfied"
        ),
    )
    ...
    memory: Literal["default", "small", "large"] = Field(
        "default", description="Controls the memory/disk usage strategy."
    )
...
class SystemNamelist(Namelist):
    ...
    ibrav: Literal[0, 1, 2, 3, -3, 4, 5, -5, 6, 7, 8, 9, -9, 91, 10, 11, 12, -12, 13, -13, 14] = (
        Field(
            ...
            description=(
                ...
                "parameters. For ibrav=0 specify the lattice vectors in CELL_PARAMETER, optionally "
                "the lattice parameter alat = celldm(1) (in a.u.) or = A (in Angstrom), or else it "
                "is taken from CELL_PARAMETERS  ibrav      structure                   "
                ...
                "(a/2,-b/2,0),  v2 = (a/2, b/2,0),  v3 = (0,0,c)  10          Orthorhombic "
                "face-centered      celldm(2)=b/a celldm(3)=c/a v1 = (a/2,0,c/2),  v2 = "
                "(a/2,b/2,0),  v3 = (0,b/2,c/2)  11          Orthorhombic body-centered      "
                "celldm(2)=b/a celldm(3)=c/a v1=(a/2,b/2,c/2),  v2=(-a/2,b/2,c/2),  "
                "v3=(-a/2,-b/2,c/2)  12          Monoclinic P, unique axis c     celldm(2)=b/a "
                "celldm(3)=c/a, celldm(4)=cos(ab) v1=(a,0,0), v2=(b*cos(gamma),b*sin(gamma),0),  "
                "v3 = (0,0,c) where gamma is the angle between axis a and b. -12          "
                "Monoclinic P, unique axis b     celldm(2)=b/a celldm(3)=c/a, celldm(5)=cos(ac) v1 "
                "= (a,0,0), v2 = (0,b,0), v3 = (c*cos(beta),0,c*sin(beta)) where beta is the angle "
                "between axis a and c  13          Monoclinic base-centered        celldm(2)=b/a "
                "celldm(3)=c/a, celldm(4)=cos(gamma) v1 = (  a/2,         0,                -c/2), "
                "v2 = (b*cos(gamma), b*sin(gamma),       0  ), v3 = (  a/2,         0,             "
                "    c/2), where gamma=angle between axis a and b projected on xy plane  -13       "
                "   Monoclinic base-centered        celldm(2)=b/a (unique axis b)                 "
                "celldm(3)=c/a, celldm(5)=cos(beta) v1 = (  a/2,       b/2,             0), v2 = ( "
                "-a/2,       b/2,             0), v3 = (c*cos(beta),   0,   c*sin(beta)), where "
                "beta=angle between axis a and c projected on xz plane IMPORTANT NOTICE: until QE "
                "v.6.4.1, axis for ibrav=-13 had a different definition: v1(old) = v2(now), "
                "v2(old) = -v1(now)  14          Triclinic                       celldm(2)= b/a, "
                "celldm(3)= c/a, celldm(4)= cos(bc), celldm(5)= cos(ac), celldm(6)= cos(ab) v1 = "
                "(a, 0, 0), v2 = (b*cos(gamma), b*sin(gamma), 0) v3 = (c*cos(beta),  "
    ...
    nbnd: int | None = Field(
        ...
        description=(
            ...
            "calculations the number of k-point, not the number of bands per k-point, is doubled  "
            "By default the number of bands is set internally: for an insulator, the number of "
            "valence bands (number of electrons / 2); for a metal, 20% more (at least 4 more)."
        ),
    )
    tot_charge: Annotated[float, Quantity(units="e", dimensionality="charge")] = Field(
        ...
        description=(
            ...
            "missing from the system, tot_charge=-1 means one additional electron, and so on.  In "
            "a periodic calculation a compensating jellium background is inserted to remove "
    ...
    tot_magnetization: float = Field(
        ...
        description=(
            ...
            "electronic magnetization. If unspecified, the tot_magnetization variable is ignored "
            "and the electronic magnetization is determined by the occupation numbers (see card "
            "OCCUPATIONS) read from input."
        ),
    )
    ecutwfc: Annotated[float | None, Quantity(units="Ry", dimensionality="energy")] = Field(
        None, description="kinetic energy cutoff for wavefunctions"
    )
    ecutrho: Annotated[float | None, Quantity(units="Ry", dimensionality="energy")] = Field(
        ...
        description=(
            ...
            "but it will introduce noise especially on forces and stress. If there are ultrasoft "
            "PP, a larger value than the default is often desirable (ecutrho = 8 to 12 times "
            "ecutwfc, typically). PAW datasets can often be used at 4*ecutwfc, but it depends on "
            "the shape of augmentation charge: testing is mandatory. The use of gradient-corrected "
            "functional, especially in cells with vacuum, or for pseudopotential without "
            "non-linear core correction, usually requires an higher values of ecutrho to be "
    ...
    q2sigma: Annotated[float, Quantity(units="Ry", dimensionality="energy")] = Field(
        ...
        description=(
            ...
            "variable-cell molecular dynamics (or in stress calculation). 'ecfixed' is the value "
            "of the constant-cutoff; 'qcutz' and 'q2sigma' are the height and the width of the "
            "energy step for reciprocal vectors whose square modulus is greater than 'ecfixed'. In "
            "the kinetic energy, G^2 is replaced by G^2 + qcutz * (1 + erf ( (G^2 - "
            "ecfixed)/q2sigma) ) See: M. Bernasconi et al, J. Phys. Chem. Solids 56, 501 (1995)"
        ),
    )
    input_dft: str | None = Field(
        ...
        json_schema_extra={"computed_default": True},
        description=(
            "Exchange-correlation functional: eg 'PBE', 'BLYP' etc See Modules/funct.f90 for "
            "allowed values. Overrides the value read from pseudopotential files. Use with care "
            "and if you know what you are doing!  Use 'PBE0' to perform hybrid functional "
            "calculation using Wannier functions. Allowed calculation: 'cp-wf' and 'vc-cp-wf' See "
            "CP specific user manual for further guidance (or in CPV/Doc/user_guide.tex) and "
            "examples in CPV/examples/EXX-wf-example. Also see related keywords starting with exx_."
        ),
    )
    ...
    lda_plus_u: bool = Field(
        False,
        description=(
            "lda_plus_u = .TRUE. enables calculation with LDA+U ('rotationally invariant'). See "
            "also Hubbard_U. Anisimov, Zaanen, and Andersen, PRB 44, 943 (1991) "
    ...
    assume_isolated: Literal["none", "makov-payne"] = Field(
        "none",
        description=(
            "Used to perform calculation assuming the system to be isolated (a molecule or a "
...
class ElectronsNamelist(Namelist):
    ...
    electron_dynamics: Literal["none", "sd", "damp", "verlet", "cg"] = Field(
        "none", description="set how electrons should be moved."
    )
    ...
    orthogonalization: Literal["ortho", "Gram-Schmidt"] = Field(
        "ortho", description="Selects the orthonormalization method for electronic wave functions."
    )
    ...
    electron_damping: float = Field(
        ...
        description=(
            ...
            ": SQRT( 0.5 * LOG( ( E1 - E2 ) / ( E2 - E3 ) ) ) where E1, E2, E3 are successive "
            "values of the DFT total energy in a steepest descent simulations. meaningful only if "
            "' electron_dynamics = 'damp"
        ),
    )
    electron_velocities: Literal["default", "zero", "change_step"] = Field(
        "default", description="Specifies how to initialise electronic velocities at restart."
    )
    electron_temperature: Literal["nose", "not_controlled"] = Field(
        "not_controlled", description="Specifies how the electronic temperature is controlled."
    )
    ...
    startingwfc: Literal["default", "random", "atomic"] = Field(
        "random", description="Specifies the initial guess for the electronic wavefunctions."
    )
    tcg: bool = Field(
        False,
        description=(
            "if .TRUE. perform a conjugate gradient minimization of the electronic states for "
            "every ionic step. It requires Gram-Schmidt orthogonalization of the electronic states."
        ),
    )
    maxiter: int = Field(
        100,
        description=(
            "maximum number of conjugate gradient iterations for conjugate gradient minimizations "
    ...
    n_inner: int = Field(
        2,
        description=(
            "number of internal cycles for every conjugate gradient iteration only for ensemble DFT"
        ),
    )
    ...
    grease: float = Field(
        1.0e0,
        description=(
            "a number <= 1, very close to 1: the damping in electronic damped dynamics is "
            "multiplied at each time step by 'grease' (avoids overdamping close to convergence: "
            "Obsolete ?) grease = 1 : normal damped dynamics"
        ),
    )
    ampre: float = Field(
        0.0e0,
...
class IonsNamelist(Namelist):
    ...
    ion_dynamics: Literal["none", "sd", "damp", "verlet"] = Field(
        ...
        description=(
            ...
            "optimisations add the CONSTRAINTS card (when the card is present the SHAKE algorithm "
            "is automatically used)."
        ),
    )
    ion_positions: Literal["default", "from_input"] = Field(
        "default", description="Selects the source of the initial atomic positions."
    )
    ion_velocities: Literal["default", "change_step", "random", "from_input", "zero"] = Field(
        "default", description="Initial ionic velocities."
    )
    ...
    remove_rigid_rot: bool = Field(
        ...
        description=(
            ...
            "isolated system. If set to true the total torque of the internal forces is set to "
            "zero by adding new forces that compensate the spurious interaction with the periodic "
            "images. This allows for the use of smaller supercells.  BEWARE: since the potential "
            "energy is no longer consistent with the forces (it still contains the spurious "
            "interaction with the repeated images), the total energy is not conserved anymore. "
            "However the dynamical and thermodynamical properties should be in closer agreement "
            "with those of an isolated system. Also the final energy of a structural relaxation "
            "will be higher, but the relaxation itself should be faster."
        ),
    )
    ion_temperature: Literal["nose", "rescaling", "not_controlled"] = Field(
        "not_controlled", description="Specifies how the ionic temperature is controlled."
    )
    ...
    nhptyp: int = Field(
        0,
        description=(
            "type of the 'massive' Nose-Hoover chain thermostat nhptyp=1 uses a NH chain per each "
            "atomic type nhptyp=2 uses a NH chain per atom, this one is useful for extremely rapid "
            "equipartitioning (equilibration is a different beast) nhptyp=3 together with nhgrp "
            "allows fine grained thermostat control NOTE: if using more than 1 thermostat per "
            "system there will be a common thermostat added on top of them all, to disable this "
            "common thermostat specify nhptyp=-X instead of nhptyp=X"
        ),
    )
    ndega: int = Field(
        0,
    ...
    ion_radius: Annotated[list[float], Quantity(units="bohr", dimensionality="length")] = Field(
        default_factory=list,
        description=(
            "ion_radius(i): pseudo-atomic radius of the i-th atomic species used in Ewald "
            "summation. Typical values: between 0.5 and 2. Results should NOT depend upon such "
            "parameters if their values are properly chosen. See also 'iesr'. (start = 1, end = "
            "ntyp)"
        ),
    )
...
class CellNamelist(Namelist):
    ...
    cell_parameters: Literal["default", "from_input"] = Field(
        "default", description="Specifies how cell parameters are initialised at restart."
    )
    ...
    cell_velocities: Literal["default", "zero"] = Field(
        "default", description="Selects the source of the initial cell velocities."
    )
    ...
    cell_temperature: Literal["nose", "not_controlled"] = Field(
        ...
        description=(
            ...
            "'not_controlled' are currently implemented in the parser; 'rescaling' is documented "
            "in the literature but not handled."
        ),
    )
    ...
    cell_dofree: str = Field(
        "all",
        description=(
            "Select which of the cell parameters should be moved:  all     = all axis and angles "
            "are moved x       = only the x component of axis 1 (v1_x) is moved y       = only the "
            "y component of axis 2 (v2_y) is moved z       = only the z component of axis 3 (v3_z) "
            "is moved xy      = only v1_x and v2_y are moved xz      = only v1_x and v3_z are "
            "moved yz      = only v2_y and v3_z are moved xyz     = only v1_x, v2_y, v3_z are "
            "moved shape   = all axis and angles, keeping the volume fixed 2Dxy    = only x and y "
            "components are allowed to change 2Dshape = as above, keeping the area in xy plane "
            "fixed volume  = isotropic variations of v1_x, v2_y, v3_z, keeping the shape fixed. "
...
class PressAiNamelist(Namelist):
    ...
    pvar: bool = Field(
        False,
        description=(
            ".true. for variable pressure calculations pressure changes linearly with time: "
    ...
    dthr: Annotated[float, Quantity(units="bohr", dimensionality="length")] = Field(
        ...
        description=(
            ...
            "'quantum' surface (typical values: 1.d-4 - 1.d-3; 50% to 100% of rho_thr) "
...
class WannierNamelist(Namelist):
    ...
    wf_switch: bool = Field(
        False,
        description=(
            "Whether to turn on the field adiabatically (adiabatic switch) if true, then nbeg is "
            "set to 0."
        ),
    )
    sw_len: int = Field(
    ...
    wfsd: int = Field(
        1,
        description=(
            "Localization algorithm for Wannier function calculation: wfsd=1  Damped Dynamics "
            "wfsd=2  Steepest-Descent / Conjugate-Gradient wfsd=3  Jocobi Rotation Remember, this "
            "is consistent with all the calwf options as well as the tolw (see below). Not a good "
            "idea to Wannier dynamics with this if you are using restart='from_scratch' option, "
            "since the spreads converge fast in the beginning and ortho goes bananas."
        ),
    )
    ...
    maxwfdt: float = Field(
        ...
        description=(
            ...
            "step size, but that may be either too small (takes forever to converge)  or too large "
            "(code goes crazy) . This option keeps the step size between wfdt and maxwfdt. In my "
            "experience 0.1 and 0.5 work quite well. (but don't blame me if it doesn't work for "
            "you)"
        ),
    )
    ...
    wf_q: float = Field(
        ...
        description=(
            ...
            "then U is computed from A. Usually a value between 1500 and 2500 works fine, but "
            "should be tested."
        ),
    )
    ...
    calwf: int = Field(
        3,
        description=(
            "Wannier Function Options, can be 1,2,3,4,5  1. Output the Wannier function density, "
            "nwf and wffort are used for this option. see below. 2. Output the Overlap matrix "
            "O_i,j=<w_i|exp{iGr}|w_j>. O is written to unit 38. For details on how O is "
            "constructed, see below. 3. Perform nsteps of Wannier dynamics per CP iteration, the "
            "orbitals are now Wannier Functions, not Kohn-Sham orbitals. This is a Unitary "
            "transformation of the occupied subspace and does not leave the CP Lagrangian "
            "invariant. Expectation values remain the same. So you will **NOT** have a constant of "
            "motion during the run. Don't freak out, its normal. 4. This option starts for the KS "
            "states and does 1 CP iteration and nsteps of Damped-Dynamics to generate  maximally "
            "localized wannier functions. Its useful when you have the converged KS groundstate "
            "and want to get to the converged Wannier function groundstate in 1 CP Iteration. 5. "
            "This option is similar to calwf 1, except that the output is the Wannier "
            "function/wavefunction, and not the orbital density. See nwf below."
        ),
    )
    nwf: int = Field(
        0,
        description=(
            "This option is used with calwf 1 and calwf 5. with calwf=1, it tells the code how "
            "many Orbital densities are to be output. With calwf=5, set this to 1(i.e calwf=5 only "
            "writes one state during one run. so if you want 10 states, you have to run the code "
            "10 times). With calwf=1, you can print many orbital densities in a single run. See "
            "also the PLOT_WANNIER card for specifying the states to be printed."
        ),
    )
    wffort: int = Field(
        40,
        description=(
            ...
            "appropriate number (e.g. 40) then the first orbital density will be output to "
            "fort.40, the second to fort.41 and so on. Note that in the current implementation, "
            "the following units are used 21,22,24,25,26,27,28,38,39,77,78 and whatever you define "
    ...
    exx_dis_cutoff: Annotated[float, Quantity(units="bohr", dimensionality="length")] = Field(
        8.0,
        description=(
            "Radial cutoff distance for including overlapping MLWF pairs in EXX calculations. See "
            "J. Chem. Theory Comput. 16, 3757â3785 (2020)."
        ),
    )
    ...
    exx_ps_rcut_pair: Annotated[float, Quantity(units="bohr", dimensionality="length")] = Field(
        ...
        description=(
            ...
            "parameter can generally be chosen as smaller than exx_ps_rcut_self. See J. Chem. "
            "Theory Comput. 16, 3757â3785 (2020)."
        ),
    )
    ...
    exx_me_rcut_pair: Annotated[float, Quantity(units="bohr", dimensionality="length")] = Field(
        ...
        description=(
            ...
            "parameter can generally be chosen as smaller than exx_me_rcut_self. See J. Chem. "
            "Theory Comput. 16, 3757â3785 (2020)."
        ),
    )
