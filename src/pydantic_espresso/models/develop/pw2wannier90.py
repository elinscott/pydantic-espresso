...
class InputppNamelist(Namelist):
    ...
    reduce_unk_factor: int | None = Field(
        ...
        description=(
            "The reduction factor per direction for reduce_unk. Default 2 means a reduction of "
            "2x2x2 = 8 of the total number of grid points. Only relevant if write_unk = .true."
        ),
    )
    wvfn_formatted: bool = Field(
        False,
        description=(
            "if .TRUE. write the wavefunctions in Fortran formatted form. Only relevant if "
            "write_unk = .true."
        ),
    )
    ...
    atom_proj_ext: bool = Field(
        False,
        description=(
            "Set to .true. to use the external pseudo-atomic wavefunctions from the files stored "
            "in atom_proj_dir as the initial projection. Only relevant if atom_proj = .true."
        ),
    )
    atom_proj_dir: Path = Field(
        ...
        description=(
            ...
            "names should be of the form SPECIES.dat, where SPECIES is the species name of the "
            "atom. For more details, see the wannier90 user guide and examples. Only relevant if "
            "atom_proj_ext = .true."
        ),
    )
    ...
    write_spn: bool = Field(
        False,
        description=(
            "Set to .true. to write out the matrix elements of spin operator S between Bloch "
            "states (non-collinear spin calculation only)."
        ),
    )
    ...
    uHu_formatted: bool = Field(  # noqa: N815
        False,
        description=(
            "Set to .true. to write uHu data as a Fortran formatted file. Only relevant if "
            "write_uHu = .true."
        ),
    )
    ...
    uIu_formatted: bool = Field(  # noqa: N815
        False,
        description=(
            "Set to .true. to write uIu data as a Fortran formatted file. Only relevant if "
            "write_uIu = .true."
        ),
    )
    write_sHu: bool = Field(  # noqa: N815
        False,
        description=(
            "Set to .true. to write out the matrix elements of < unk | s H | umk+b >, which is "
            "used in the Ryoo's method to compute spin Hall conductivity. For more details, see "
            "the wannier90 user guide and examples."
        ),
    )
    sHu_formatted: bool = Field(  # noqa: N815
        False,
        description=(
            "Set to .true. to write sHu data as a Fortran formatted file. Only relevant if "
            "write_sHu = .true."
        ),
    )
    write_sIu: bool = Field(  # noqa: N815
        False,
        description=(
            "Set to .true. to write out the matrix elements of < unk | s | umk+b >, which is used "
    ...
    sIu_formatted: bool = Field(  # noqa: N815
        False,
        description=(
            "Set to .true. to write sIu data as a Fortran formatted file. Only relevant if "
            "write_sIu = .true."
        ),
    )
    ...
    irr_bz: bool = Field(
        ...
        description=(
            ...
            "from the standard full BZ case, the files will use the extension names "
            "iamn/immn/ieig, respectively. For more details, see the wannier90 user guide and "
            "examples."
        ),
    )
    ...
    read_sym: bool = Field(
        ...
        description=(
            ...
            "When read_sym = .true., an additional input seedname.sym is required. Only relevant "
            "if write_dmn = .true."
        ),
    )
