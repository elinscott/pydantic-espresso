"""Pydantic model for the input of `pw.x` version `qe-6.7MaX-Release`.
...
class ControlNamelist(Namelist):
    ...
    wf_collect: bool = Field(
        True,
        description="This flag controls the way wavefunctions are stored to disk :  .TRUE.  collect wavefunctions from all processors, store them into the output data directory 'outdir'/'prefix'.save The resulting format is portable to a different number of processors, or different kind of parallelization  .FALSE. OBSOLETE - NO LONGER IMPLEMENTED do not collect wavefunctions, leave them in temporary local files (one per processor). The resulting format is readable only on the same number of processors and with the same kind of parallelization used to write it.  Note that this flag has no effect on reading, only on writing.",
    )
