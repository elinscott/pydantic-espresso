"""Pydantic model for the input of `pw.x` version `qe-6.4.1`.
...
class ControlNamelist(Namelist):
    ...
    nstep: int | None = Field(
        None,
        description="number of molecular-dynamics or structural optimization steps performed in this run",
    )
    ...
    disk_io: Literal[None, "high", "medium", "low", "none"] = Field(
        None,
        description="Specifies the amount of disk I/O activity:  Default is 'low' for the scf case, 'medium' otherwise. Note that the needed RAM increases as disk I/O decreases! It is no longer needed to specify 'high' in order to be able to restart from an interrupted calculation (see restart_mode) but you cannot restart in disk_io=='none",
    )
...
class SystemNamelist(Namelist):
    ...
    ecutfock: float | None = Field(
        None,
        description="Kinetic energy cutoff (Ry) for the exact exchange operator in EXX type calculations. By default this is the same as ecutrho but in some EXX calculations significant speed-up can be found by reducing ecutfock, at the expense of some loss in accuracy. Must be .gt. ecutwfc. Not implemented for stress calculation. Use with care, especially in metals where it may give raise to instabilities.",
    )
    nosym: bool = Field(
        False,
    ...
    lda_plus_u_kind: int = Field(
        0,
        description="Specifies the type of DFT+U calculation:  0   simplified version of Cococcioni and de Gironcoli, PRB 71, 035105 (2005) (https://journals.aps.org/prb/abstract/10.1103/PhysRevB.71.035105), using Hubbard_U  1   rotationally invariant scheme of Liechtenstein et al., using Hubbard_U and Hubbard_J",
    )
    ...
    report: int = Field(
        100,
        description="Number of iterations after which the program writes all the atomic magnetic moments.",
    )
...
class ElectronsNamelist(Namelist):
    ...
    def map_diagonalization(cls, v: str) -> str:
        ...
        mapping = {"cg-serial": "cg-serial", "david-serial": "cg-serial"}
        return mapping.get(v, v)
    ...
    diagonalization: Literal["david", "cg", "cg-serial"] = Field(
