...
class InputNamelist(Namelist):
    ...
    use_irr_q: bool = Field(
        ...
        description=dedent(
            """\
            If .true., assume that the q points are on the irreducible grid. In this case, the data
            in ahc_dir must have been actually generated from a calculation on an irreducible q
            grid. Off-diagonal self-energies cannot be computed if use_irr_q = .true."""
        ),
    )
    ahc_nbnd: int = Field(
        ...,
        description=dedent(
            """\
            Number of bands for which the electron self-energy is to be computed. Must be identical
            to ahc_nbnd of the ph.x run with electron_phonon='ahc'."""
        ),
    )
    ...
    eta_eV: Annotated[float, Quantity(units="eV", dimensionality="energy")] = Field(  # noqa: N815
        ...,
        description=dedent(
            """\
    ...
    temp_kelvin: Annotated[float, Quantity(units="K", dimensionality="temperature")] = Field(
        ..., description="Temperature at which the electron self-energy is calculated."
    )
    efermi_eV: Annotated[float, Quantity(units="eV", dimensionality="energy")] = Field(  # noqa: N815
        ..., description="Fermi energy of the electrons."
    )
    ...
    skip_upper: bool = Field(
        ...
        description=dedent(
            ...
            Debye-Waller self-energy to include only the low-energy band contribution. (Corresponds
            to the second term (lower Fan + lower DW) of Eq. (G1-revised) of J.-M. Lihm and C.-H.
            Park, PRX 12, 039901(E) (2022).) If .false., calculate the contribution of both the
            high-energy and low-energy bands. In this case, ahc_upfan_iq#.bin files must be present
            in ahc_dir."""
        ),
    )
    skip_dw: bool = Field(
    ...
    adiabatic: bool = Field(
        ...
        description=dedent(
            ...
            inaccurate and even divergent in some materials (S. Ponc̩ et al., J. Chem. Phys. 143,
            102813 (2015)). Therefore, this keyword should be used only for experimental or
            debugging purposes."""
        ),
    )
    amass_amu: Annotated[list[float], Quantity(units="amu", dimensionality="mass")] = Field(
