...
class InputppNamelist(Namelist):
    ...
    output: str = Field(
        "xsf",
        description="a string describing the output format to be performed, allowed values: 'xsf', 'grd'  xsf     xcrysden format grd     GRD gaussian 3D grid format",
    )
    outdir: Path = Field(
        Path("./"),
    ...
    ldynamics: bool = Field(
        False,
        description="This logical flag control the processing of atoms trajeactory.  .TRUE.  process CP trajectory files and generata a trajectory file for xcrysden (.axsf)  .FALSE. do not proces trajectory",
    )
    lpdb: bool = Field(
        False,
        description="This logical flag control the generation of a pdb file.  .TRUE.  generate a pdb file containing positon and cell of the simulated system  .FALSE. do not generate pdb file",
    )
