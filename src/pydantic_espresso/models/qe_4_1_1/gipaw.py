"""Pydantic model for the input of `gipaw.x` version `qe-4.1.1`.
...
class InputgipawNamelist(Namelist):
    ...
    job: str = Field(
        "nmr",
        description="select calculation to perform. The possible values are:  'f-sum'        check the f-sum rule 'nmr'          compute the magnetic suscept. and NMR chemical shifts 'g_tensor'     compute the EPR g-tensor 'efg'          compute the electric field gradients at the nuclei",
    )
    ...
    filfield: str | None = Field(
        None, description="for saving the induced magnetic field (valence only)"
    )
    read_recon_in_paratec_fmt: bool = Field(
        False, description="for reading reconstruction data from Paratec converted pseudopotentials"
    )
    use_nmr_macroscopic_shape: bool | None = Field(
        None,
        description="correct the chemical shift by taking into account the macroscopic shape of the sample",
    )
    nmr_macroscopic_shape: list[float] | None = Field(
        None, description="tensor for the macroscopic shape correction"
    )
    spline_ps: bool = Field(
        True,
