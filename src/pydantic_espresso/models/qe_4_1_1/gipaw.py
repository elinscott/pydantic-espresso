...
class InputgipawNamelist(Namelist):
    ...
    conv_threshold: float | None = Field(None, description="convergence threshold for the diagonalization and for the Green's function solver")
    ...
    filcurr: str = Field("", description="for saving the induced current (valence only)")
    filfield: str = Field("", description="for saving the induced magnetic field (valence only)")
    ...
    nmr_macroscopic_shape(3,3): float | None = Field(None, description="tensor for the macroscopic shape correction")
...
class GIPAWEspressoInput(EspressoInput):
    ...
    inputgipaw: InputgipawNamelist = Field(default_factory=InputgipawNamelist)
