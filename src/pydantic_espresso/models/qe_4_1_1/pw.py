"""Pydantic model for the input of `pw.x` version `qe-4.1.1`.
...
class ControlNamelist(Namelist):
    ...
    calculation: str = Field(
        "scf",
        description="a string describing the task to be performed: 'scf', 'nscf', 'bands', 'relax', 'md', 'vc-relax', 'vc-md', 'neb', 'smd', 'metadyn' (vc = variable-cell).",
    )
...
class SystemNamelist(Namelist):
    ...
    tot_charge: float = Field(
        0.0,
        description="total (ionic + electronic) charge in the system. By default the cell is assumed to be neutral.  In a periodic calculation a compensating jellium background is inserted to remove divergences if the cell is not neutral.",
    )
    multiplicity: int | None = Field(
        None,
        description="spin multiplicity (2s+1). 1 is singlet, 2 for doublet etc. Note that this fixes the final value of the magnetization. if unspecified then multiplicity variable is ignored and the ammount of electronic magnetization is determined during the self-consistent cycle. Do not specify both multiplicity and tot_magnetization.",
    )
    tot_magnetization: float | None = Field(
        None,
        description="total majority spin charge - minority spin charge. Used to impose a specific total electronic magnetization. If unspecified then tot_magnetization variable is ignored and the ammount of electronic magnetization is determined during the self-consistent cycle. Do not specify both multiplicity and tot_magnetization.  YES, there is redundancy between multiplicity and tot_magnetization! However these variables are likely appropriated for different situations: 'multiplicity' is more natural when considering a molecule; 'tot_magnetization' is more natural when considering a solid.",
    )
...
class IonsNamelist(Namelist):
    ...
    wfc_extrapolation: str = Field(
        "none",
        description="Used to extrapolate the wavefunctions from preceding ionic steps.  'none'        :  no extrapolation  'first_order' :  extrapolate the wave-functions with first-order formula.  'second_order':  as above, with second order formula.",
    )
