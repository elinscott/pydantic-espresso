...
class ControlNamelist(Namelist):
    ...
    calculation: str = Field(
        "cp",
        description="a string describing the task to be performed: 'cp', 'scf', 'nscf', 'relax', 'vc-relax', 'vc-cp', 'cp-wf'  (vc = variable-cell).",
    )
    ...
    iprint: int = Field(
        10,
        description="Number of steps between successive writings of relevant physical quantities to standard output and to files 'fort.3?' or 'prefix.???' depending on 'prefix' parameter",
    )
    tstress: bool = Field(
        False,
    ...
    prefix: str = Field(
        "cp", description="prepended to input/output filenames: prefix.pos, prefix.vel, etc."
    )
    ...
    tabps: bool = Field(
        False,
        description=".true. to compute the volume and/or the surface of an isolated system for finete pressure/finite surface tension calculations (PRL 94, 145501 (2005) (https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.94.145501); JCP 124, 074103 (2006)).",
    )
    ...
    disk_io: str | None = Field(
        None,
        description="high', 'default'  'high': CP code will write Kohn-Sham wf files and additional information in data-file.xml in order to restart with a PW calculation or to use postprocessing tools.",
    )
...
class SystemNamelist(Namelist):
    ...
    input_dft: str | None = Field(
        None,
        description="Exchange-correlation functional: eg 'PBE', 'BLYP' etc See Modules/functionals.f90 for allowed values. Overrides the value read from pseudopotential files. Use with care and if you know what you are doing!",
    )
