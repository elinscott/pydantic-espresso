...
class Input_pw2bgwNamelist(Namelist):
    ...
    symm_type: str = Field("cubic", description="cubic' | 'hexagonal' type of crystal system, 'cubic' for space groups 1 ... 142 and 195 ... 230 and 'hexagonal' for space groups 143 ... 194. Only used if ibrav = 0 in a pw.x run. Written to BerkeleyGW WFN, RHO, VXC and VNL files but no longer used (except in SAPO code in BerkeleyGW). You can use the default value for all systems. Don't set to different values in different files for the same system or you will get errors in BerkeleyGW.")
    ...
    input_dft: str = Field("sla+pz", description="exchange-correlation functional, same as in pw.x. Not used if vxcg_flag = .FALSE. and vxc_flag = .FALSE.")
    exx_flag: bool = Field(False, description="set to .TRUE. when using EXX. Not used if vxcg_flag = .FALSE. and vxc_flag = .FALSE.")
    vnlg_flag: bool = Field(False, description="write Kleinman-Bylander projectors in G-space to BerkeleyGW VNL file. Only used in SAPO code in BerkeleyGW")
    vnlg_file: str = Field("VNL", description="name of BerkeleyGW VNL output file. Only used in SAPO code in BerkeleyGW. Not used if vnlg_flag = .FALSE.")
