"""Pydantic model for the input of `pw_export.x` version `qe-5.2.1`.
...
class InputppNamelist(Namelist):
    ...
    prefix: str | None = Field(
        None,
        description="the first part of the name of all the file written by the code should be equal to the value given in the main calculations.",
    )
    outdir: Path = Field(
        Path("./"), description="the scratch directory where the massive data-files are written"
    )
    pseudo_dir: Path = Field(
        Path("./"), description="directory containing pseudopotential (PP) files"
    )
    single_file: bool = Field(False, description="if .TRUE. one-file output is produced")
    ascii: bool = Field(
        False, description="if .TRUE. output files are textual, otherwise they are partly binary."
    )
    pp_file: str = Field(
        "prefix.export/",
        description="Output file.  If it is omitted, a directory 'prefix.export/' is created in outdir and some output files are put there. Anyway all the data are accessible through the 'prefix.export/index.xml' file which contains implicit pointers to all the other files in the export directory. If reading is done by the IOTK library all data appear to be in index.xml even if physically it is not.",
    )
    uspp_spsi: bool = Field(
        False,
    ...
    psfile: list[str] | None = Field(
        None,
        description="files containing i-th pseudopotential, where i=1, N_type. PP numbering must follow the ordering defined in the input of pw.x (start = 1, end = ntyp)",
    )
