"""Pydantic model for the input of `cppp.x` version `qe-7.2-omp5-1.1-before_merge`.
...
class InputppNamelist(Namelist):
    ...
    output: str = Field(
        "xsf",
        description="a string describing the output format to be performed, allowed values: 'xsf', 'xyz'  xsf     xcrysden format xyz     XMOL format",
    )
    outdir: Path = Field(
        Path("./"),
    ...
    lforces: bool = Field(
        False,
        description="This logical flag control the processing of forces.  .TRUE.  extract forces from trajectory files and write them to xcrysden file  .FALSE. do not process forces",
    )
