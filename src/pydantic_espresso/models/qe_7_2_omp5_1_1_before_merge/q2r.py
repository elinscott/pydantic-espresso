"""Pydantic model for the input of `q2r.x` version `qe-7.2-omp5-1.1-before_merge`.
...
class InputNamelist(Namelist):
    ...
    write_lr: bool | None = Field(
        None,
        description="set to .true. to write long-range IFC into q2r IFC file. This is required when enforcing asr='all' for infrared- active solids in matdyn. An additional column will be written for long-range part of IFC for text format, while a tag named IFC_LR will be created for xml format.",
    )
