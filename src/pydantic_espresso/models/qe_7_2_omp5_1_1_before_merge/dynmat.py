...
class InputNamelist(Namelist):
    ...
    remove_interaction_blocks: bool = Field(
        False,
        description="If .true. lines and rows corresponding to atoms that are not displaced are set to 0. This can be used for ph.x calculations with nat_todo set in ph.x, to remove the interaction between the diplaced and fixed parts of the system.",
    )
    axis: int = Field(
