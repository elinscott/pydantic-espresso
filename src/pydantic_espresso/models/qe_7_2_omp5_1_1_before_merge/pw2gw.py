"""Pydantic model for the input of `pw2gw.x` version `qe-7.2-omp5-1.1-before_merge`.
...
class InputppNamelist(Namelist):
    ...
    vxcdiag: bool = Field(
        False,
        description="if .TRUE. calculates the expectation value of the exchange and correlation potential on all the Kohn-Sham states and write it into the 'vxcdiag.dat' file.",
    )
    vkb: bool = Field(
        False,
        description="if .TRUE. use Kleyman-Bylander projectors to write additional informatio into fort.15 file (Still in development)",
    )
    Emin: float = Field(
        0.0,
    ...
    DeltaE: float = Field(
        0.05, description="Energy step with which the dielectric function is calculated (in eV)"
    )
