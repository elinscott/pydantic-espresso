"""Pydantic model for the input of `ld1.x` version `qe-7.2-omp5-1.1-before_merge`.
...
class InputpNamelist(Namelist):
    ...
    nlcc: bool = Field(False, description="If .true. produce a PP with the nonlinear core correction of Louie, Froyen, and Cohen [PRB 26, 1738 (1982) (https://journals.aps.org/prb/abstract/10.1103/PhysRevB.26.1738)].")
    ...
    rho0: float = Field(0.0, description="Charge at the origin: when the Rappe-Rabe-Kaxiras-Joannopoulos method with 3 Bessel functions fails, specifying rho0 > 0 may allow to override the problem (using 4 Bessel functions). Typical values are in the order of 0.01-0.02")
