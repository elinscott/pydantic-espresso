"""Pydantic model for the input of `postahc.x` version `qe-7.2-omp5-1.1-before_merge`.
...
class InputNamelist(Namelist):
    ...
    nat: int | None = Field(
        None,
        description="Number of atoms. Must be identical to nat of the preceding pw.x and ph.x runs.",
    )
    nq: int | None = Field(
        None,
        description="Number of q points. Must be identical to that of the preceding ph.x run with electron_phonon='ahc'.",
    )
    ...
    eta: float | None = Field(
        None,
        description="Magnitude of the small imaginary component included to smooth the energy denominators, in Ry.",
    )
    ...
    skip_upperfan: bool = Field(
        False,
        description="If .true., skip calculation of the upper Fan self-energy. If .false., ahc_upfan_iq#.bin files must be present in ahc_dir.",
    )
    skip_dw: bool = Field(
        False,
