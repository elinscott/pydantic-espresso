"""Pydantic model for the input of `all_currents.x` version `e6621af`.
...
class Energy_currentNamelist(BaseModel):
   """Pydantic model for the `Energy_current` namelist."""
    ...
    re_init_wfc_2: bool = Field(False, description="If true initializes, as specified in the ELECTRON namelist of the PW section, the wavefunctions before the second ground state calculation, then compute the charge density. Otherwise use the last calculated wavefunctions. Note that if three_point_derivative is false, this has no effect.")
    ...
    three_point_derivative: bool = Field(True, description="If true calculates three ground stated: one at t - delta_t /2, one at t and one at t + delta_t/2. Obviously it needs more computer time, but the derivative should be better.")
    n_repeat_every_step: int = Field(1, description="Number of repetition of the full current calculation for each step. If > 1, the file file_output + '.stat' is written with some statistics. Note that if you don't specify at least re_init_wfc_1 ,this may be useless. You may want to specify startingwfc = 'random' in the ELECTRONS namelist.")
    n_workers: int = Field(0, description="The calculation over all the trajectory is splitted in n_workers chunks. Then to run the code over all the trajectory you must run n_workers input files each one with a different worker_id, from 0 to n_workers - 1 . Those inputs can run at the same time in the same folder. The worker_id will be appended to the outdir folder and to the file_output input variables, so you can safely run all the inputs in the same directory at the same time.")
...
class ALL_CURRENTSEspressoInput(BaseModel):
    """Pydantic model for the input of `all_currents.x.`"""

    Energy_current: Energy_currentNamelist
