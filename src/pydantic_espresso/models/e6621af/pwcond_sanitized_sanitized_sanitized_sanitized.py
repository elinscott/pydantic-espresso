"""Pydantic model for the input of `pwcond.x` version `e6621af`.
...
class InputcondNamelist(BaseModel):
   """Pydantic model for the `Inputcond` namelist."""

    outdir: str | None = Field(None, description="temporary directory (as in PWscf)")
    ...
    tran_prefix: str = Field("none", description="if tran_prefix is specified the program will save partial results of a transmission calculation (ikind .GE. 1) in a specific directory (outdir/tran_prefix.cond_save)")
    max_seconds: float | None = Field(None, description="jobs stops after max_seconds elapsed time (wallclock time). It can be enabled only if tran_prefix is specified.")
    ...
    loop_ek: bool | None = Field(None, description="if .t. the energy loop is outside the k-point loop")
    ...
    tk_plot: int | None = Field(None, description="if > 0, plot T(kx,ky) at each energy in the region [tk_plot x full BZ]")
    llocal: bool | None = Field(None, description="if .t. calculations are done with only local part of PP")
    bdl: float | None = Field(None, description="right boundary of the left lead (left one is supposed to be at 0) (in units of lattice parameter 'alat' defined in the scf run)")
    bds: float | None = Field(None, description="right boundary of the scatt. reg. (left one is at 0 if prefixs is used and = bdl if prefixt is used) (in units of lattice parameter 'alat' defined in the scf run)")
    bdr: float | None = Field(None, description="right boundary of the right lead (left one is at 0 if prefixr is used and = bds if prefixt is used) (in units of lattice parameter 'alat' defined in the scf run)")
    ...
    start_e: int = Field(1, description="if start_e > 1, the scattering problem is solved only for those energies with index between start_e and last_e in the energy list.  NOTE: start_e <= last_e and start_e <= nenergy must be satisfied")
    last_e: int | None = Field(None, description="index of the last energy to be computed. If last_e > nenergy, then last_e will be automatically set to nenergy.")
    start_k: int = Field(1, description="if start_k > 1, the scattering problem is solved only for those k-points with index between start_k and last_k in the k-point list. In order to recover the full transmission (i.e. integrated over the full Brillouin Zone) at the end, perform the partial runs specifying a value for tran_prefix (the restart directory), then put all the partial transmission files 'transmission_k#_e#' inside a unique restart directory and run pwcond.x with recover=.TRUE. (without specifying any value for start_k and last_k).  NOTE: start_k <= last_k must be satisfied and start_k must also not be greater than the actual number of k-point in the list (if you compute the grid automatically by specifying the grid size and shifts, you can use kpoints.x to check that number).")
    last_k: int | None = Field(None, description="index of the last k-point to be computed. If last_k is bigger than the actual number of points in the list, then it will be set to that number.")
...
class PWCONDEspressoInput(BaseModel):
    """Pydantic model for the input of `pwcond.x.`"""

    Inputcond: InputcondNamelist
