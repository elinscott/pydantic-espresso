...
class HPEspressoInput(EspressoInputTemplate):
...
value of the ", description="Directory containing input, output, and scratch files; must be the same as specified in the calculation of the unperturbed system.")
    ...
    docc_thr: float = Field( 5.D-5
         , description="Threshold for a comparison of unperturbed occupations which is needed for the selection of atoms which must be perturbed. Can be used only when")
    start_q: int = Field( 1
         , description="Computes only the q points from")
    last_q: int = Field( number of q points
         , description="Computes only the q points from")
    sum_pertq: bool = Field(False, description="If it is set to .true. then the HP code will collect pieces of the response occupation matrices for all q points. This variable should be used only when")
    ...
    conv_thr_chi: float = Field( 1.D-5
    ...
    thresh_init: float = Field( 1.D-14
    ...
    niter_max: int = Field( 100
         , description="Maximum number of iterations in the iterative solution of the linear-response Kohn-Sham equations.")
    alpha_mix(i): float = Field( alpha_mix(1)=0.3
         , description="Mixing parameter (for the i-th iteration) for updating the response SCF potential using the modified Broyden method. See: D.D. Johnson,")
    nmix: int = Field( 4
         , description="Number of iterations used in potential mixing using the modified Broyden method. See: D.D. Johnson,")
    num_neigh: int = Field( 6
    ...
    lmin: int = Field( 2
         , description="Minimum value of the orbital quantum number of the Hubbard atoms starting from which (and up to the maximum l in the system) Hubbard V will be written to the file parameters.out.")
    rmax: float = Field( 100.D0
    ...
    no_metq0: bool = Field(False, description="If .true. the metallic response term at q=0 is ignored (i.e. the last term in Eq. (22) in")
