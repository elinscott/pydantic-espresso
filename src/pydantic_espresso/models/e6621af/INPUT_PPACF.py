...
class PPACFEspressoInput(EspressoInputTemplate):
    """Pydantic model for the input of `ppacf.x.`"""

    prefix: str = Field(..., description="prefix of files saved by program pw.x prepended to input/output filenames: prefix.ecnl, prefix.tcnl, etc.")
    outdir: str = Field("
value of the ", description="directory containing the output data from pw.x, i.e. the same as in pw.x")
    n_lambda: int = Field(
1
