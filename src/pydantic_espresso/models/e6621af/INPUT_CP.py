"""Pydantic model for the input of `cp.x` version `e6621af`.
...
class CPEspressoInput(EspressoInputTemplate):
    """Pydantic model for the input of `cp.x.`"""

    calculation: str = Field(" 'cp'
    ...
    restart_mode: str = Field(" 'restart'
...
current directory ('./') otherwise
    ...
    saverho: bool = Field(..., description="This flag controls the saving of charge density in CP codes: If  .TRUE.        save charge density to restart dir, If .FALSE. do not save charge density.")
    prefix: str = Field(" 'cp'
         ", description="prepended to input/output filenames and restart folders:   prefix.pos : atomic positions   prefix.vel : atomic velocities   prefix.for : atomic forces   prefix.cel : cell parameters   prefix.str : stress tensors   prefix.evp : energies   prefix.hrs : Hirshfeld effective volumes (ts-vdw)   prefix.eig : eigen values   prefix.nos : Nose-Hoover variables   prefix.spr : spread of Wannier orbitals   prefix.wfc : center of Wannier orbitals   prefix.ncg : number of Poisson CG steps (PBE0)   prefix_ndw.save/ : write restart folder   prefix_ndr.save/ : read restart folder where ndr and ndw are the integers number described below")
    ndr: int = Field( 50
         , description="The restart files are read from the folder   outdir/prefix_ndr.save/ where outdir, prefix and ndr are the input variables described in this document")
    ndw: int = Field( 50
         , description="The restart files are written, if ndw > 0, in the folder   outdir/prefix_ndw.save/ where outdir, prefix and ndw are the input variables described in this document")
    tabps: bool = Field(False, description=".true. to compute the volume and/or the surface of an isolated system for finite pressure/finite surface tension calculations (")
    ...
    etot_conv_thr: float = Field( 1.0D-4
    ...
    forc_conv_thr: float = Field( 1.0D-3
    ...
    ekin_conv_thr: float = Field( 1.0D-6
    ...
    memory: str = Field(" 'default'
         ", description="'small': NO LONGER IMPLEMENTED SINCE v.6.3          memory-saving tricks are implemented. Currently:          - the G-vectors are sorted only locally, not globally          - they are not collected and written to file          For large systems, the memory and time gain is sizable          but the resulting data files are not portable - use it          only if you do not need to re-read the data file")
    pseudo_dir: str | None = Field(None, description="directory containing pseudopotential files")
...
for a metal, 20% more (minimum 4 more)
    ...
    tot_magnetization: float = Field( -1 [unspecified]
    ...
    ecutwfc: float = Field(..., description="kinetic energy cutoff (Ry) for wavefunctions")
    ecutrho: float = Field( 4 * ecutwfc
    ...
    occupations: str = Field(..., description="a string describing the occupation of the electronic states. Allowed values are 'fixed' (default) and 'ensemble'. In the case of conjugate gradient style of minimization of the electronic states, if occupations is set to 'ensemble', this allows ensemble DFT calculations for metallic systems.")
    degauss: float = Field( 0.D0 Ha
         , description="parameter for the smearing function, only used for ensemble DFT calculations. Hartree atomic units")
    smearing: str = Field(..., description="a string describing the kind of occupations for electronic states in the case of ensemble DFT (occupations == 'ensemble' ); possible values are: 'gaussian', 'fermi-dirac', 'hermite-delta', 'gaussian-splines', 'cold-smearing', 'marzari-vanderbilt', '0', '-1'. Warning: only 'gaussian' is tested.")
    nspin: int = Field( 1
         , description="nspin = 1 :  non-polarized calculation (default)  nspin = 2 :  spin-polarized calculation, LSDA              (magnetization along z axis)")
    ecfixed: float = Field( 0.0
         , description="")
    qcutz: float = Field( 0.0
         , description="")
    q2sigma: float = Field( 0.1
    ...
    input_dft: str = Field(" read from pseudopotential files
    ...
    exx_fraction: float = Field( it depends on the specified functional
         , description="Fraction of EXX for hybrid functional calculations. In the case of             input_dft='PBE0', the default value is 0.25. This entry overrides the default (as well as the restart file) value of a given functional.")
    lda_plus_u: bool = Field(False, description="lda_plus_u = .TRUE. enables calculation with LDA+U                   ('rotationally invariant'). See also Hubbard_U.                   Anisimov, Zaanen, and Andersen,")
    vdw_corr: str = Field(" 'none'
         ", description="Type of Van der Waals correction. Allowed values:     'grimme-d2', 'Grimme-D2', 'DFT-D', 'dft-d': semiempirical Grimme's DFT-D2.     Optional variables: 'london_s6', 'london_rcut'     S. Grimme, J. Comp. Chem. 27, 1787 (2006),     V. Barone et al., J. Comp. Chem. 30, 934 (2009).      'TS', 'ts', 'ts-vdw', 'ts-vdW', 'tkatchenko-scheffler': Tkatchenko-Scheffler      dispersion corrections with first-principle derived C6 coefficients      Optional variables: 'ts_vdw_econv_thr', 'ts_vdw_isolated'      See A. Tkatchenko and M. Scheffler, Phys. Rev. Lett. 102, 073005 (2009)      J. Hermann et al., J. Chem. Phys. 159, 174802 (2023),")
    london_s6: float = Field( 0.75
         , description="global scaling parameter for DFT-D. Default is good for PBE.")
    london_rcut: float = Field( 200
    ...
    ts_vdw_econv_thr: float = Field( 1.D-6
    ...
    assume_isolated: str = Field(" 'none'
         ", description="Used to perform calculation assuming the system to be         isolated (a molecule of a clustr in a 3D supercell).          Currently available choices:          'none' (default): regular periodic calculation w/o any correction.          'makov-payne', 'm-p', 'mp' : the Makov-Payne correction to the                  total energy is computed.                  Theory:                  G.Makov, and M.C.Payne,                  'Periodic boundary conditions in ab initio                  calculations' , Phys.Rev.B 51, 4014 (1995)   var nextffield -type INTEGER {   default { 0 }   info {       Number of activated external ionic force fields.       See Doc/ExternalForceFields.tex for further explanation and parameterizations   } }")
    electron_maxstep: int = Field( 100
         , description="maximum number of iterations in a scf step")
    electron_dynamics: str = Field(" 'none'
    ...
    orthogonalization: str = Field(" 'ortho'
    ...
    ion_positions: str = Field(" 'default'
    ...
    ion_velocities: str = Field(" 'default'
         ", description="initial ionic velocities 'default'     : restart the simulation with atomic velocities read                 from the restart file 'change_step' : restart the simulation with atomic velocities read                 from the restart file, with rescaling due to the                 timestep change, specify the old step via")
    ion_damping: float = Field( 0.2D0
    ...
    ion_temperature: str = Field(" 'not_controlled'
    ...
    tolp: float = Field( 100.D0
         , description="tolerance (in Kelvin) of the rescaling. When ionic temperature differs from 'tempw' more than 'tolp' apply rescaling. meaningful only with")
    nhpcl: int = Field( 1
    ...
    greasp: float = Field( 1.D0
         , description="same as 'grease', for ionic damped dynamics.")
    cell_parameters: str = Field(..., description="'default'      : restart the simulation with cell parameters read                from the restart file or 'celldm' if                'restart = 'from_scratch'' 'from_input'   : restart the simulation with cell parameters                from standard input.                ( see the card 'CELL_PARAMETERS' )")
    cell_dynamics: str = Field("
'pr'      if ", description="set how cell should be moved 'none'      : cell is kept fixed 'sd'        : steepest descent algorithm is used to optimise the               cell 'damp-pr'   : damped dynamics is used to optimise the cell               ( Parrinello-Rahman method ). 'pr'        : standard Verlet algorithm is used to propagate               the cell ( Parrinello-Rahman method ).")
    cell_velocities: str = Field(..., description="'zero'      : restart setting cell velocity to zero 'default'   : restart using cell velocity of the previous run")
    cell_damping: float = Field( 0.1D0
...
0.75*Tot_Mass/pi**2/Omega**(2/3) for Wentzcovitch MD
    ...
    cell_temperature: str = Field(" 'not_controlled'
    ...
    P_in: float = Field( 0.D0
         , description="only if pvar = .true. initial value of the external pressure (GPa)")
    P_fin: float = Field( 0.D0
         , description="only if pvar = .true. final value of the external pressure (GPa)")
    Surf_t: float = Field( 0.D0
         , description="Surface tension (in a.u.; typical values 1.d-4 - 1.d-3)")
    rho_thr: float = Field( 0.D0
         , description="threshold parameter which defines the electronic charge density isosurface to compute the 'quantum' volume of the system (typical values: 1.d-4 - 1.d-3) (corresponds to alpha in")
    dthr: float = Field( 0.D0
         , description="thikness of the external skin of the electronic charge density used to compute the 'quantum' surface (typical values: 1.d-4 - 1.d-3; 50% to 100% of rho_thr) (corresponds to Delta in")
    ...
    wf_friction: float = Field( 0.3D0
         , description="Damping coefficient for Damped-Dynamics.")
    nsteps: int = Field( 20
         , description="Number of Damped-Dynamics steps to be performed per CP iteration.")
    tolw: float = Field( 1.D-8
    ...
    exx_neigh: int = Field( 60
         , description="An initial guess on the maximum number of neighboring (overlapping) MLWFs.")
    exx_dis_cutoff: float = Field( 8.0
         , description="Radial cutoff distance (in bohr) for including overlapping MLWF pairs in EXX calculations. See J. Chem. Theory Comput. 16, 3757â3785 (2020).")
    exx_poisson_eps: float = Field( 1.0D-6
         , description="Poisson solver convergence criterion during computation of the EXX potential.")
    exx_use_cube_domain: bool = Field(False, description="Use cubic instead of spherical subdomains as local supports during computation of the EXX potential. If set to .TRUE., the spherical domain radii (exx_ps_rcut_self, exx_ps_rcut_pair, exx_me_rcut_self, exx_me_rcut_pair) will be treated as half of the side length of the cubic subdomain.")
    exx_ps_rcut_self: float = Field( 6.0
         , description="Radial cutoff distance (in bohr) to compute the self EXX energy. This distance determines the radius of the Poisson sphere centered at a given MLWF center, and should be large enough to cover the majority of the MLWF charge density. See J. Chem. Theory Comput. 16, 3757â3785 (2020).")
    exx_ps_rcut_pair: float = Field( 5.0
    ...
    exx_me_rcut_self: float = Field( 10.0
    ...
    exx_me_rcut_pair: float = Field( 7.0
