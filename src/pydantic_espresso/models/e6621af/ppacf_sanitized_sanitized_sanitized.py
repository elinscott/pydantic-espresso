"""Pydantic model for the input of `ppacf.x` version `e6621af`.
...
class PpacfNamelist(BaseModel):
   """Pydantic model for the `Ppacf` namelist."""
    ...
    outdir: str = Field("value of the", description="directory containing the output data from pw.x, i.e. the same as in pw.x")
    ...
    vdW_analysis: int | None = Field(None, description="Select type of vdw kernel table used in ppacf coupling-constant scaling analysis of nonlocal-correlations in vdW-DF versions: - vdW_analysis = 0: Full Ecnl kenel of vdW-DF method - vdW_analysis = 1: The cumulant- or susceptibility-Ecnl kernel component - vdW_analysis = 2: The pure-vdW-Ecnl kernel component See IOP JCPM (2020) for presentation of the latter two (non-default) options")


class PPACFEspressoInput(BaseModel):
