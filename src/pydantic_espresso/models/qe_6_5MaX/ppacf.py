"""Pydantic model for the input of `ppacf.x` version `qe-6.5MaX`.
...
class PpacfNamelist(Namelist):
    ...
    pseudo_dir: Path = Field(
        default_factory=get_pseudo_dir,
        description="Directory containing pseudopotential files (and vdw kernel table).",
    )
    vdw_table_name: str = Field(
        "vdW_kernel_table", description="The vdw kernel table (in Quantum ESPRESSO format)."
    )
