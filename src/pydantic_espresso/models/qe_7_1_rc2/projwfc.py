...
class ProjwfcNamelist(Namelist):
    ...
    lwrite_overlaps: bool = Field(False, description="if .true., the overlap matrix of the atomic orbitals prior to orthogonalization is written to 'atomic_proj.xml'. Does not work together with parallel diagonalization: for parallel runs, use 'mpirun -np N projwfc.x -nd 1 ...")
    lbinary_data: bool = Field(False, description="CURRENTLY DISABLED. if .true., write atomic projections to a binary file.")
