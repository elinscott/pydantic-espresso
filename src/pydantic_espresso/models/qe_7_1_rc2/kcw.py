...
class ControlNamelist(Namelist):
    ...
    calculation: Literal[None, "wann2kcw", "screen", "ham"] = Field(
    ...
    spread_thr: float | None = Field(
        None,
        description="NOT IMPLMENTED YET. Two or more Wannier functions are considered identical if their spread (self-hartree) differ by less than spread_thr. Requires check_spread = .true.",
    )
...
class ScreenNamelist(Namelist):
    ...
    eps_inf: float = Field(
        1.0e0,
        description="The macroscopic dielectric constant. Needed for the Gygi-Baldereschi scheme if l_vcut = .TRUE. Typically from exp or from a ph.x calculation. NOTA BENE: This would be equivalent to a Makov-Payne correction. it is perfectly fine for cubic systems. Not so for anisotropic dielectric matrices.",
    )
...
class HamNamelist(Namelist):
    ...
    do_bands: bool = Field(
        False,
        description="If .true. the interpolated band structure is computed along a path secified with the K_POINTS card ( see PW documentation https://www.quantum-espresso.org/Doc/INPUT_PW.html#idm1346 )",
    )
