...
class PlotNamelist(Namelist):
    ...
    output_format: int | None = Field(
        None,
        description="(ignored on 1D plot) 0  format suitable for gnuplot   (1D) 1  format suitable for contour.x (2D) 2  format suitable for plotrho   (2D) 3  format suitable for XCRYSDEN  (2D or user-supplied 3D region) 4  format suitable for gOpenMol  (3D) (formatted: convert to unformatted *.plt) 5  format suitable for XCRYSDEN  (3D, using entire FFT grid) 6  format as gaussian cube file  (3D) (can be read by many programs) 7  format suitable for gnuplot   (2D) x, y, f(x,y)",
    )
    fileout: str | None = Field(None, description="name of the file to which the plot is written")
    interpolation: str = Field(
        "fourier", description="type of interpolation: 'fourier' or 'bspline' (EXPERIMENTAL!)"
    )
