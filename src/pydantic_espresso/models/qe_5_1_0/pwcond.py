...
class InputcondNamelist(Namelist):
    ...
    bds: float | None = Field(
        None,
        description="right boundary of the scatt. reg. (left one is at 0 if prefixs is used and = bdl if prefixt is used)",
    )
    bdr: float | None = Field(
        None,
        description="right boundary of the right lead (left one is at 0 if prefixr is used and = bds if prefixt is used)",
    )
    nz1: int | None = Field(
