"""Pydantic model for the input of `postahc.x` version `qe-7.5-rc.1`.
...
class InputNamelist(Namelist):
    ...
    use_irr_q: bool = Field(
        False,
        description="If .true., assume that the q points are on the irreducible grid. In this case, the data in ahc_dir must have been actually generated from a calculation on an irreducible q grid. Off-diagonal self-energies cannot be computed if use_irr_q = .true.",
    )
    ahc_nbnd: int | None = Field(
        None,
        description="Number of bands for which the electron self-energy is to be computed. Must be identical to ahc_nbnd of the ph.x run with electron_phonon='ahc'.",
    )
    ...
    eta_eV: float | None = Field(
        None,
        description="Magnitude of the small imaginary component included to smooth the energy denominators, in eV.",
    )
    temp_kelvin: float | None = Field(
        None, description="Temperature in Kelvins at which the electron self-energy is calculated."
    )
    efermi_eV: float | None = Field(None, description="Fermi energy of the electrons in eV.")
    ahc_win_min_eV: float = Field(
        -1.0e8, description="Lower bound of active AHC window for the lower Fan term in eV."
    )
    ahc_win_max_eV: float = Field(
        1.0e8, description="Upper bound of active AHC window for the lower Fan term in eV."
    )
    skip_upper: bool = Field(
        False,
        description="If .true., skip calculation of the upper Fan self-energy. Also, truncate the Debye-Waller self-energy to include only the low-energy band contribution. (Corresponds to the second term (lower Fan + lower DW) of Eq. (G1-revised) of J.-M. Lihm and C.-H. Park, PRX 12, 039901(E) (2022).) If .false., calculate the contribution of both the high-energy and low-energy bands. In this case, ahc_upfan_iq#.bin files must be present in ahc_dir.",
    )
    skip_dw: bool = Field(
        False,
