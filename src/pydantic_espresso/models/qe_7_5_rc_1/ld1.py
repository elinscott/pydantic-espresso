"""Pydantic model for the input of `ld1.x` version `qe-7.5-rc.1`.
...
class TestNamelist(Namelist):
    ...
    configts: list[str] | None = Field(
        None,
        description="A string array containing the test electronic configuration. configts(nc), nc=1,nconf, has the same syntax as for config but only VALENCE states must be included. If configts(i) is not set, the electron configuration is read from the cards following the namelist. (start = 1, end = nconf)",
    )
