"""Pydantic model for the input of `cp.x` version `qe-7.5-rc.1`.
...
class SystemNamelist(Namelist):
    ...
    occupations: str | None = Field(
        None,
        description="a string describing the occupation of the electronic states. Allowed values are 'fixed' (default) and 'ensemble'. In the case of conjugate gradient style of minimization of the electronic states, if occupations is set to 'ensemble', this allows ensemble DFT calculations for metallic systems.",
    )
    ...
    smearing: str | None = Field(
        None,
        description="a string describing the kind of occupations for electronic states in the case of ensemble DFT (occupations == 'ensemble' ); possible values are: 'gaussian', 'fermi-dirac', 'hermite-delta', 'gaussian-splines', 'cold-smearing', 'marzari-vanderbilt', '0', '-1'. Warning: only 'gaussian' is tested.",
    )
    nspin: int = Field(
        1,
    ...
    vdw_corr: str = Field(
        "none",
        description="Type of Van der Waals correction. Allowed values:  'grimme-d2', 'Grimme-D2', 'DFT-D', 'dft-d': semiempirical Grimme's DFT-D2. Optional variables: 'london_s6', 'london_rcut' S. Grimme, J. Comp. Chem. 27, 1787 (2006), V. Barone et al., J. Comp. Chem. 30, 934 (2009).  'TS', 'ts', 'ts-vdw', 'ts-vdW', 'tkatchenko-scheffler': Tkatchenko-Scheffler dispersion corrections with first-principle derived C6 coefficients Optional variables: 'ts_vdw_econv_thr', 'ts_vdw_isolated' See A. Tkatchenko and M. Scheffler, Phys. Rev. Lett. 102, 073005 (2009) J. Hermann et al., J. Chem. Phys. 159, 174802 (2023), doi:10.1063/5.0170972 (https://doi.org/10.1063/5.0170972)  'XDM', 'xdm': Exchange-hole dipole-moment model. Optional variables: 'xdm_a1', 'xdm_a2' (implemented in PW only) A. D. Becke and E. R. Johnson, J. Chem. Phys. 127, 154108 (2007) A. Otero de la Roza, E. R. Johnson, J. Chem. Phys. 136, 174109 (2012)  Note that non-local functionals (eg vdw-DF) are NOT specified here but in 'input_dft",
    )
...
class ElectronsNamelist(Namelist):
    ...
    pre_state: bool = Field(
        False,
        description="if .TRUE. perform the precondition of the CG gradient using the kinetic energy of the state.",
    )
    n_inner: int = Field(
        2,
...
class IonsNamelist(Namelist):
    ...
    tolp: float = Field(
        100.0e0,
        description="tolerance (in Kelvin) of the rescaling. When ionic temperature differs from 'tempw' more than 'tolp' apply rescaling. meaningful only with ion_temperature = 'rescaling' or with ion_velocities='change_step', where it specifies the old timestep",
    )
    nhpcl: int = Field(
        1,
...
class WannierNamelist(Namelist):
    ...
    exx_use_cube_domain: bool = Field(
        False,
        description="Use cubic instead of spherical subdomains as local supports during computation of the EXX potential. If set to .TRUE., the spherical domain radii (exx_ps_rcut_self, exx_ps_rcut_pair, exx_me_rcut_self, exx_me_rcut_pair) will be treated as half of the side length of the cubic subdomain.",
    )
