...
class ProjwfcNamelist(Namelist):
    ...
    lwrite_overlaps: bool = Field(False, description="if .true., the overlap matrix of the atomic orbitals prior to orthogonalization is written to the atomic_proj datafile. Does not work together with linear-algebra diagonalization: run as 'mpirun -np N projwfc.x -nd 1 ...")
    lbinary_data: bool = Field(False, description="if .true., the atomic_proj datafile is written in binary fmt. Currently disabled.")
    ...
    irmin: list[int] | None = Field(None, description="first point of the given box  BEWARE: irmin is a 2D array of the form: irmin(3,n_proj_boxes)")
    irmax: list[int] | None = Field(None, description="last point of the given box; ( 0 stands for the last point in the FFT grid )  BEWARE: irmax is a 2D array of the form: irmax(3,n_proj_boxes)")
