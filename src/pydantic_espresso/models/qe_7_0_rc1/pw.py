"""Pydantic model for the input of `pw.x` version `qe-7.0-rc1`.
...
class SystemNamelist(Namelist):
    ...
    vdw_corr: Literal["none", "grimme-d2", "grimme-d3", "TS", "mbd_vdw", "XDM"] = Field(
        "none",
...
class ElectronsNamelist(Namelist):
    ...
    diago_rmm_ndim: int = Field(
        4, description="Max dimension  of the iterative subspace for RMM-DIIS diagonalization"
    )
    diago_rmm_conv: bool = Field(
        False,
        description="If .TRUE. during the SCF loop the RMM-DIIS is reiterated until all bands are converged or up to a max of 8 times.",
    )
    efield: float = Field(
        0.0e0,
